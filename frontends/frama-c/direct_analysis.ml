...
module With_focusing = Codex.Domains.Memory_domains.With_focusing

module Log = Tracelog.Make(struct let category = "Direct_analysis" end);;

(* An hash-consed callstack. *)

(* Note: this does not seems to be no longer useful. What we need is
   only to connect to Tracelog callstacks, to easily view in which
   circonstance an expression has some value. *)
module Callstack = struct
  ...
  let rec pretty fmt = function
    ...
    | Frame{id;calling_function;calling_instr;parent} ->
      Format.fprintf fmt "{%d;%a;%a;%a}"
        id
        Kernel_function.pretty calling_function
        Cil_datatype.Kinstr.pretty calling_instr
        pretty parent
  ...
  module FrameHash = Weak.Make(struct
      ...
      let hash = function
        | (Frame {id = _; calling_function =cfa; calling_instr=cia; parent=parenta})  ->
          Hashing.hash3 (Kernel_function.hash cfa) (Cil_datatype.Kinstr.hash cia) (hash parenta)
        | Root -> 0
    end)
  ...
  let get() =
    ...
    (* Remove what is inside the current function. *)
    let rec loop = function
      | [] -> []
      | ((Function _)::_) as l -> l
      | _::l -> loop l in
    let loc_stack = loop loc_stack in
    let rec convert = function
      | [] -> Root
      | [Function calling_function] -> cons calling_function Kglobal Root
      | (Function calling_function)::(Kinstr calling_instr)::l ->
        let parent = convert l in
        cons calling_function calling_instr parent
      | _ -> assert false       (* The stack should look like this. *)
    in
    convert loc_stack
...
module Analyze(Domain:With_focusing.S_with_types) = struct
  ...
  module Analyze_Function = struct
    ...
    let join_state statea stateb =
      ...
      then (
        ...
        let Domain.Context.Result(_,tup,deserialize2) =
          Domain.serialize_memory_and_cache ~widens:false statea.context.ctx statea.mem stateb.context.ctx stateb.mem []
            (true,Domain.Context.empty_tuple ()) in
        ...
        let tmp = {mem;
                   ...
                   context = {loop_nesting_level; ctx; kinstr=Kglobal }} in
        Log.debug (fun p -> p "------------------------------------------------------") ;
        ...
        let Domain.Context.Result(inc,tup,deserialize_mem) =
          Domain.serialize_memory ~widens:false ctxa statea.mem ctxb stateb.mem (true,Domain.Context.empty_tuple ()) in
          ...
          VarMap.fold_on_diff2 statea.var_addresses stateb.var_addresses
            ...
            (fun vi a b (Domain.Context.Result(inc,acc,d_map) as res) ->
               ...
               | _ ->
                 ...
                 let Domain.Context.Result(inc,acc,d_addr) = Domain.serialize_binary ~widens:false ~size ctxa a ctxb b
                     (inc,acc) in
        ...
        {mem;
         var_addresses = map;
         string_addresses = statea.string_addresses;
         context = {loop_nesting_level; ctx; kinstr=Kglobal}}
    ...
    let join_state_option statea stateb = match statea, stateb with
      ...
      | Some a, Some b ->
        ...
        let open C2Codex in let open Codex_options in
        Log.trace ~loc:(Location.JoinPoint(a.context.kinstr, b.context.kinstr))
          (fun p -> p "Join")
    ...
    let fixpoint_step_state ~loop_id previous next =
      ...
      let Domain.Context.Result(is_included,tup,deserialize2) =
        Domain.serialize_memory_and_cache ~widens:true previous.context.ctx previous.mem
      ...
      let tmp =
        {mem; var_addresses = previous.var_addresses;
         ...
         context = {loop_nesting_level=previous.context.loop_nesting_level;
                    ctx; kinstr=Kglobal }} in
      tmp, is_included ;;

    let fixpoint_step_state ~loop_id previous next =
      let open C2Codex in
      Log.trace ~loc:(Codex_options.Location.JoinPoint(previous.context.kinstr, next.context.kinstr))
        (fun p -> p "fixpoint_step_state")
        (fun () -> fixpoint_step_state ~loop_id previous next)
              ...
              begin match state with
                ...
                | Some state ->
                  ...
                  let context = C2Codex.{kinstr = e.Interpreted_automata.edge_kinstr;
                                         ...
                                         loop_nesting_level = state.context.loop_nesting_level} in
                  Log.trace ~loc:(Codex_options.Location.Kinstr kinstr)
                    (fun p -> p "Doing transition at kinstr %a" Cil_datatype.Kinstr.pretty kinstr)
                    (fun () ->
                    ...
                    let rec loop i (state:C2Codex.state) =
                      ...
                      | Some(state') ->
                        ...
                        let do_fixpoint_step () =
                          ...
                          Log.debug (fun p -> p "Before widening, state A = %a" C2Codex.pretty_state state) ;
                          Log.debug (fun p -> p "and state B = %a" C2Codex.pretty_state state') ;
                          (* let Domain.Context.Result(included,tup,deserialize2) =
                             Domain.serialize_memory state.context.ctx state.mem state'.context.ctx state'.mem
                              (true,Domain.Context.empty_tuple ()) in *)
                          let Domain.Context.Result(included,tup,deserialize2) =
                            Domain.serialize_memory_and_cache ~widens:true arg_ctx state.mem state'.context.ctx state'.mem []
                              (true,Domain.Context.empty_tuple ()) in
                          let close, f =
                          ...
                          in
                          close,state,out_ctx in

                        (* Frama_c_alarms.current_kinstr := None ; *)
                        let close,state,out_ctx =
                          Log.trace
                            ~loc:(Codex_options.Location.JoinPoint(state.context.kinstr, state'.context.kinstr))
                            (fun p -> p "Iteration %d: Fixpoint step" i)
    ...
    module Analyze_Body_Wto = struct
      ...
      module Graph = Fixpoint.Fixpoint_graph.Make(struct
          ...
          (* The module Interpreted_automata.Transition is no longer exported in recent Frama-C.
             If needed for debugging purposes, the code can be temporarily copied. *)
          let transition_to_string (v,a) = ""
          (* Format.asprintf "%a" Interpreted_automata.Transition.pretty v *)
      ...
      module AbstractDomain(Arg:sig
        ...
        type loop_id = Domains.Sig.Widening_Id.t
        ...
        let enter_loop state =
          ...
          let loop_id = Domains.Sig.Widening_Id.fresh() in
    ...
    let analyze_function_body funcall entry_state kf =
      if  Codex_options.FixpointUseRegexp.get() (* false *) (* true *)
      then Analyze_Body_Regexp.analyze_function_body funcall entry_state kf
      else Analyze_Body_Wto.analyze_function_body funcall entry_state kf

    (** Whenever we call a transfer function that may emit an alarm,
        we need to attribute it to some function. [inside_function]
        delimits the scope to what it means to be "inside a
        function". *)
    let inside_function kf f =
      Log.trace ~loc:(Codex_options.Location.Function kf)
        (fun p -> p "Inside function %a" Kernel_function.pretty kf)
        f
    ...
    let entryfuncall = fun kf args state (rtyp : Types.TypedC.typ) ->
      ...
      let state = Log.trace
          ~loc:(Codex_options.Location.FunctionReturn)
          (fun p -> p "On function return")
          return in
      (* Frama_c_alarms.current_kinstr := None ; *)
      retval, state
  ...
  (**this reset alarms, call this only at the very end of the analysis*)
  let dump_alarms outc =
    ...
    (*the alarms are reseted, do not call this function lightly*)
    C2Codex.reset_alarms ();
    Frama_c_alarms.reset_alarms ()


  let dump_extra outc =
    Printf.fprintf outc "UF Unions performed: %d\n%!"  !Terms.Builder.nb_unions;
    Printf.fprintf outc "UF Max Clusters: %d\n%!"
      (Hashtbl.fold (fun _ j m -> max m j) Terms.Builder.sizes 0);
    Printf.fprintf outc "UF Binary terms: %d\n%!" !Terms.Builder.nb_binary_terms
  ...
  (** Dump the evaluation of assertions. This reset assertions, call this only at the end of the analysis *)
  ...
  let print_webapp file =
    let outc = open_out file in
    Fun.protect ~finally:(fun () -> close_out outc) @@ fun () ->
    (* DONE: First, move the webapp into a new repository, utils/gui.  *)
    let interface:Codex.Gui.Interface.marshalled =
      let open Gui.Interface in
      let tracelog = Binarytrace.read_all_items () in
      let disassembly = None in
      let cfg = None in
      let source =
        let file = Ast.get() in
        let text = Format.asprintf "%a" Printhtml2.pp_file file in
        Some {text} in
      { cfg; disassembly; tracelog; source }
    in
    Gui.Print_html_webapp.print_html_webapp outc interface

    (* Then: I will just dump a file with an empty interface.
       Maybe I can have option types to say what information is present.
    *)
    (* Then: dump a tracelog output. But I should be careful with execution time there. *)
    (* Then: also dump a parsetree.  *)


  let dump_html func_name =
    if Codex_options.HtmlDump.get () then
      let htmlfile = Format.asprintf "%s.html" func_name in
      let webappfile = Format.asprintf "%s.webapp.html" func_name in
      print_html htmlfile;
      print_webapp webappfile;
    else ()
  ...
  let run kf =
    begin match (C2Codex.initial_state_ret kf Root) with
      | Some state,args,rtyp ->
        ...
        let finally () =
          (* Dump html file even if the analysis fails. *)
          dump_html func_name
        in
        ...
        begin
          ...
          | Failure msg ->
            ...
            exit 1
          | e ->
            Codex_log.error "Unexpected exception: %s" (Printexc.to_string e);
            Codex_log.error "Backtrace: %s" (Printexc.get_backtrace ());
            raise e
        end
      | None, _, _ -> exit 1
    end
