...
module type Registers = sig
  module Domain : Codex.Domains.Sig.BASE
  val registers : (string * Units.In_bits.t) list
  val initial_value : Domain.Context.t -> string * Units.In_bits.t -> Domain.binary
end

module type S = functor
  (Domain : Codex.Domains.Memory_domains.With_focusing.S_with_types) -> sig
