...
module type StateS = sig
  module Domain : Codex.Domains.Memory_domains.With_focusing.S_with_types
  ...
  val reset : (Loader_elf.Img.t, 'a, 'b, 'c) Loader.t_pack -> Domain.Context.t ->
              t
  val get : size:Units.In_bits.t -> t -> string -> Domain.binary
  val set : t -> string -> Domain.binary -> t
  val assume : Domain.boolean -> t -> t
  (* val assume_pred : size:int -> Codex.Types.TypedC.Pred.t
...
module Create():sig
  module Numeric_simple : Codex.Domains.Sig.BASE
  module Numeric : Codex.Domains.Sig.BASE
  module Domain : Codex.Domains.Memory_domains.With_focusing.S_with_types
  ...
  module Make (Reg : Arch_settings.Registers with module Domain = Domain) : sig
    ...
    (** Analyse a DBA instruction starting from a state. [idx] is the
        position of the expression, and is used to precisely track the
        location in the source code. *)
    val expr : Dba.Expr.t Syntax_tree.located -> State.t -> (Domain.binary * State.t) option

    (** Analyse a DBA instruction starting from a state. [idx] is the
        index of the DBA instruction in the dhunk, and is used to
        precisely track the location in the source code. *)
    val instr : State.t -> Dba.Instr.t Syntax_tree.located -> (jump_target * State.t) list
