...
: S = struct
  ...
  let rec last = function
    ...
    | [x] -> x
    | _ :: xs -> last xs
  (* Conversion functions used for the graph type from graph_types.ml*)
  (*let vertex_to_light_vertex block_map id v = 
    let lead_addr = fst v in
    let call_stack = snd v in
    let light_lead_addr = Virtual_address.to_int64 lead_addr in
    let last_addr = match Hashtbl.find block_map lead_addr with
    | result -> result |> last |> Virtual_address.to_int64
    | exception Not_found -> Int64.zero
    in
    let light_call_stack = List.map Virtual_address.to_int64 call_stack in

    V_hashable.{id; head=light_lead_addr; tail=last_addr; call_stack=light_call_stack; collapsed=No}
    
  let compute_back_edges cfg tree: Graph_types.Edge_set.t =
    Cfg.fold_edges (fun v1 v2 acc ->
      if (Tree.is_parent tree v2 v1) then
        Graph_types.Edge_set.add (Virtual_address.to_int64(fst v1), Virtual_address.to_int64(fst v2)) acc
      else acc
    ) cfg Graph_types.Edge_set.empty*)
      
(*  let is_member elt stack =
    let mem = ref false in
    Stack.iter (fun i -> if (i=elt) then mem := true) stack;
    !mem*)
  ...
  let convert_cfg_to_light_cfg (block_map: (Virtual_address.t, Virtual_address.t list) Hashtbl.t) cfg (entry_node:Cfg.V.t) = 
    (* DFS to visit the old cfg from the entry node *)
(*    let node_path = Stack.create () in (* used to find ancestors to know whether an edge is a back edge or not*)*)
    let id = ref (-1) in
    let node_map = Hashtbl.create 50 in (*Map from old nodes to new light nodes (needed to avoid infinite loop)*)
    
    let rec visit node path =
      if Hashtbl.mem node_map node then
        Hashtbl.find node_map node  (* return already created light node *)
      else (
        (*Stack.push node node_path;*)
        incr id;
        let va, cs = node in        
        ...
        (* We add the node to the map before processing the successors,
           so that cycles can link to this node. *)
        Hashtbl.add node_map node light_node;
        
        (* Now process successors *)
        (*if (List.length (Cfg.succ cfg node) = 0) then 
          (let _ = Stack.pop node_path in ());*) 
        let succs = List.fold_left (fun acc succ ->
          let edge_info = {back=List.mem succ path} in
          let succ_light_node = visit succ (node :: path) in  (* will either get or create the corersponding light node *)
...
module Wto_light = Fixpoint.Wto.Make (struct 
  ...
  let dump_html trace ~ret_addr entry_node bbg block_map html_filename hashtbl img (wto: Cfg.V.t Fixpoint.Wto.component list ref) () =
    ...
    let () =
      ...
      (* the wto was computed for the instruction graph, not the basic block graph so it has too many nodes*)
