...
module Must0_Must1 = struct
  ...
  module Binary_Forward = struct
    ...
    let bimul_singleton ~(size:In_bits.t) (x0,x1) k =
      if(is_power_of_two k)
      then (Z.extract (Z.( * ) x0 k) 0 (size:>int),
            Z.extract (Z.( * ) x1 k) 0 (size:>int))
      else BL.top ~size
    ;;
    ...
    let bismod ~(size:In_bits.t) ((x0,x1) as x) ((y0,y1) as y) =
      match testbit x ((size:>int) - 1), testbit y ((size:>int) - 1) with
      ...
      | _ -> begin
          ...
          then BL.bottom ~size
            ...
            (* Put to top all the bits above k. *)
            let mask0 = Z.sub (Z.shift_left Z.one k) Z.one in
            let mask1 = Z.extract (Z.(~!) mask0) 0 (size:>int) in
    ...
    let bisdiv ~size (x0,x1) ((y0,y1) as y) =
      ...
      then
        ...
        then BL.bottom ~size
          ...
          match testbit (x0,x1) ((size:>int) - 1) with
    ...
    let bshl_singleton ~(size:In_bits.t) (x0,x1) y =
      (Z.extract (Z.shift_left x0 y) 0 (size:>int),
       Z.extract (Z.shift_left x1 y) 0 (size:>int))
    ;;


    let blshr_singleton ~(size:In_bits.t) (x0,x1) y =
      (Z.extract (Z.shift_right x0 y) 0 (size:>int),
       Z.extract (Z.shift_right x1 y) 0 (size:>int))
    ;;
    ...
    let do_shift f ~(size:In_bits.t) x ((y0,y1) as y) =
      (* Check that size is a power of two. *)
      assert((size:>int) land ((size:>int) - 1) == 0);
      assert((size:>int) <= 64);          (* We don't want to do this on huge values. *)

      (* Mask the last bits. *)
      let nzsizem1 = Z.lognot @@ Z.of_int ((size:>int) - 1) in
      ...
      then BL.singleton ~size Z.zero
        ...
        let zset = concretize (band ~size y (BL.singleton ~size (Z.of_int @@ (size:>int) - 1))) in
    ...
    let bashr  ~(size:In_bits.t) x ((y0,y1) as y) =
      (* Check that size is a power of two. *)
      assert((size:>int) land ((size:>int) - 1) == 0);
      assert((size:>int) <= 64);          (* We don't want to do this on huge values. *)
      ...
      let must_be_larger_than_size = (not @@ Z.equal Z.zero @@ Z.(land) y1 nzsizem1) in
      let overflow_value =
        match testbit x ((size:>int) - 1) with
  ...
  module Binary_Backward = struct
    ...
    let bconcat ~size1 ~size2 x y r =
      ...
      inter_refines y @@ Binary_Forward.bextract ~oldsize ~index:In_bits.zero ~size:size2 r)

    let bextract ~size ~index ~oldsize x r =
      (* Add top bits before and after if needed. *)
      let r =
        if index == In_bits.zero then r
        else Binary_Forward.bconcat ~size1:size ~size2:index r (BL.top ~size:index)
      in
      let r =
        if In_bits.(index + size) == oldsize
        then r
        else
          let missing_size = In_bits.(oldsize - (index + size)) in
          Binary_Forward.bconcat ~size1:missing_size ~size2:In_bits.(index + size) (BL.top ~size:missing_size) r
      in
      inter_refines x r
    ;;

    let buext ~(size:In_bits.t) ~(oldsize:In_bits.t) x r =
      let (r0,r1) = r in
      (* If something in the upper bits of r is known to be 1,
         the result is bottom.  Maybe not worth to check. *)
      let upper1 = Z.extract r1 (oldsize:>int) ((size:>int) - (oldsize:>int)) in
      if not @@ Z.equal Z.zero upper1
      then inter_refines x @@ BL.bottom ~size:oldsize
      else inter_refines x (Binary_Forward.bextract ~size:oldsize ~oldsize:size ~index:In_bits.zero r)
    ;;
    ...
    let update_according_to_high_bits ~bigsize ~(smallsize:In_bits.t) ((r0,r1) as r) =
      match test_high_bits ~size:bigsize r (1 + (bigsize:>int) - (smallsize:>int)) with
        | true,true -> BL.bottom ~size:bigsize
        | true,false ->
           let clear_bit = Z.(lognot) @@ Z.shift_left Z.one (smallsize:>int) in
           let r = (Z.(land) r0 clear_bit, Z.(land) r1 clear_bit) in
           r
        | false,true ->
           let set_bit = Z.shift_left Z.one (smallsize:>int) in
    ...
    let bsext ~size ~oldsize x r =
      ...
      inter_refines x (Binary_Forward.bextract ~size:oldsize ~oldsize:size ~index:In_bits.zero r)
    

    let shift_right_introduce_top ~size x k =
      if k == 0 then x
      else
        let k = In_bits.of_int k in
        Binary_Forward.bconcat 
          ~size1:k (BL.top ~size:k) 
          ~size2:In_bits.(size-k) (Binary_Forward.bextract ~index:k ~size:In_bits.(size-k) ~oldsize:size x)
    ;;
    ...
    let shift_left_introduce_top ~size x k =
      if k == 0 then x
        ...
        let k = In_bits.of_int k in
        Binary_Forward.bconcat 
        ~size1:In_bits.(size-k) (Binary_Forward.bextract ~index:In_bits.zero ~size:In_bits.(size-k) ~oldsize:size x)
        ~size2:k (BL.top ~size:k) 
    ;;


    let do_shift f ~(size:In_bits.t) x y r =
      assert(is_power_of_two @@ Z.of_int (size:>int));
      ...
      let sizem1 = (Z.of_int @@ (size:>int) - 1) in
    ...
    let bashr ~(size:In_bits.t) x y r =
      assert(is_power_of_two @@ Z.of_int (size:>int));
          ...
          let (newx,newy) = ZSet.fold (fun y (newx,newy) ->
               ...
               let r =
                 ...
                 then r
                   ...
                   let smallsize = (In_bits.of_int ((size:>int) - Z.to_int y)) in
                   update_according_to_high_bits ~bigsize:size ~smallsize r 
               in
    ...
    let bisdiv ~(size:In_bits.t) x y r =
      if testbit x ((size:>int) - 1) = Zero
         && testbit y ((size:>int) - 1) = Zero
