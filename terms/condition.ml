...
module ConditionCudd = struct
  ...
  let pretty fmt x = C_bdd.print pp_print_var(* Format.pp_print_int *) fmt x
...
module MakeConditionMapCuddMTBDD(L:Condition_map.L) = struct
  ...
  let rec find t cond cur =
    ...
    | Cond.C_bdd.False -> (* L.bottom *) assert false
    | Cond.C_bdd.True ->
      let find_all x =
  ...
  let refine ~inter (t:L.t CuddMTBDD.t) ~(cond:ConditionCudd.t) ?notcond (value:L.t) =
    ...
    let rec refine t ~cond =
      ...
      let res =
        ...
        | Cond.C_bdd.True ->     (* Replace all. *)
          replace_all value t
        | Cond.C_bdd.False -> t (* replace none *)
        | Cond.C_bdd.Ifte(var1,then1,else1) -> begin (* Replace some *)
            match t with
                    ...
                    let res =
                      ...
                      then
                        ...
                        | Cond.C_bdd.Ifte(var1,then1,else1) -> begin
                            ...
                            | Cond.C_bdd.True -> refine then2 ~cond:c1(* mk c1 (replace_all value then2) then2  *)
                            | Cond.C_bdd.False -> refine else2 ~cond:c1(* mk c1 (replace_all value else2) else2 *)
