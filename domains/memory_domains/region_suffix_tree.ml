...
module MakeOffset (Scalar: Sig.BASE) = struct
  ...
  module Binary_Forward = struct
    include Assert_false_domain.Binary.Binary_Forward
  ...
  let offset_empty _  = empty_suffix
  ...
  let offset_pretty ctx fmt x = pp (Scalar.binary_pretty ~size:(Codex_config.ptr_size ()) ctx) fmt x
  ...
  let serialize_offset ~widens ctxa a ctxb b acc =
        ...
        (fun i sa sb (Context.Result(inc,acc,d_map) as res) ->
            ...
            | _ ->
              let empty_if_none ctx = function
                | Some x -> x
                | None -> (Scalar.binary_empty ~size:(Codex_config.ptr_size ()) ctx,empty_suffix)
              in
              ...
              let Context.Result(inc,acc,d_e) =
                ...
                else Scalar.serialize_binary ~widens ~size:(Codex_config.ptr_size ()) ctxa ea ctxb eb (inc,acc)
              in
              Context.Result(inc, acc, fun ctx tup ->
  ...
  let nondet_binary ~size ctx v1 v2 =
    let Context.Result(_,tup,deserialize) =
      Scalar.serialize_binary ~widens:false ~size ctx v1 ctx v2 (true,Context.empty_tuple ())
    in
  ...
  let to_linear_exp ~size ctx x =
    let rec loop {leaf;offsets; indices} =
      ...
      (* Should be true, but this is a work around because some code do shifts
         with a negative offset (they should not)      *)
      let nuw = false in
      let acc = IntMap.fold (fun k (e,x) acc ->
          let k = Scalar.Binary_Forward.biconst ~size (Z.of_int k) ctx in
          let ke = Scalar.Binary_Forward.bimul ~size ~flags:(Operator.Flags.Bimul.pack ~nsw:false ~nuw:true) ctx k e in
          (Scalar.Binary_Forward.biadd ~size ~flags:(Operator.Flags.Biadd.pack ~nsw:false ~nuw ~nusw:false) ctx ke @@ loop x)::acc
        ) indices acc in
      let acc = IntMap.fold (fun k x acc ->
          let k = Scalar.Binary_Forward.biconst ~size (Z.of_int k) ctx in
          (Scalar.Binary_Forward.biadd ~size ~flags:(Operator.Flags.Biadd.pack ~nsw:false ~nuw ~nusw:false) ctx k @@ loop x)::acc
        ) offsets acc in
      match acc with
      | [a] -> a
  ...
  let to_linear_exp ~size ctx x =
    Log.trace (fun p -> p "to_linear_exp ~size:%d %a" (In_bits.to_int size) (offset_pretty ctx) x)
      (fun () -> to_linear_exp ~size ctx x)
  ;;
  ...
  let bisub ~size ctx a b =
    Log.trace (fun p -> p "bisub (%a) (%a)" (offset_pretty ctx) a (offset_pretty ctx) b) @@ fun () ->
    let ba = to_linear_exp ~size ctx a in
    let bb = to_linear_exp ~size ctx b in
    Scalar.Binary_Forward.bisub ~size ~flags:(Operator.Flags.Bisub.pack ~nsw:true ~nuw:false ~nusw:false) ctx ba bb
  ;;
      ...
      * else *)
      let indices = indices |> IntMap.mapi (fun k (e1,s1) ->
        (* When "entering" array elements, we cannot go beyond their size.
            Actually sometimes we can; e.g. * (struct pimpon * ) &char_buffer[i];
      ...
      let new_indices =
        ...
        | (e1, s1) ->
          if leaf then begin
            ...
            let add = Scalar.Binary_Forward.biadd ~size ~flags:(Operator.Flags.Biadd.pack ~nsw:true ~nuw:false ~nusw:false) ctx with_zero e in
            ...
            if s1 == only_leaf_no_bound then
              ...
              let add = Scalar.Binary_Forward.biadd ~size ~flags:(Operator.Flags.Biadd.pack ~nsw:true ~nuw:false ~nusw:false) ctx e1 e in
              ...
              if ( IntMap.is_empty s1.offsets && IntMap.is_empty s1.indices) then begin
                ...
                let _add = Scalar.Binary_Forward.biadd ~size ~flags:(Operator.Flags.Biadd.pack ~nsw:true ~nuw:false ~nusw:false) ctx e1 e in
  ...
  let fold_crop ~size ctx x ~inf ~sup f acc =
    ...
    let rec loop ~inf ~sup to_add { leaf; offsets; indices; min; max} acc =
      ...
      let acc = IntMap.fold (fun k (i,t) acc ->
          ...
          let res = Scalar.Query.Binary_Lattice.fold_crop_unsigned ~size bin ~inf:ival_min ~sup:ival_max acc (fun i acc ->
  ...
  (* Check that each array index is within its bounds. Note that
      ...
      would be incorrect!) *)
  let within_bounds ~(size:In_bits.t) ctx off =
          ...
          let v = match sup with
            ...
            | CBound m ->
              ...
              Scalar.Binary_Forward.bisle ~size:ptrsize ctx e
                ...
                (Scalar.Binary_Forward.biconst ~size:ptrsize (Z.of_int @@ (m - (size_in_bytes:>int))/k) ctx)
          in
          Bool.nondet acc v
        ) indices acc in
  ...
  module Offset_transfer_functions = struct
    ...
    let offset_shift ~offset ~max = bshift ~size:(Codex_config.ptr_size ()) ~offset ~max
...
module MakePrev(Scalar: Sig.BASE) = struct
  ...
  (* The abstract domain can be seen as follow.  Offsets in a region
     ...
     possible values into a tree commes from the syntax (e.g. .x when
     x is the first field is different than [a] when a is zero). *)
  module Operable_Value = MakeOffset(Scalar)
  ...
  module type Array_Arg = sig
    ...
    val load: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Value.binary (* Value.binary *)

    (* Copying part of a cell. *)
    val extract: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> t

    val serialize:widens:bool -> memsize:In_bytes.t -> Value.Context.t -> t -> Value.Context.t ->  t -> 'a Value.Context.in_acc -> (t,'a) Value.Context.result

    val pretty:memsize:In_bytes.t -> Value.Context.t -> Format.formatter -> t -> unit

    val union: memsize:In_bytes.t -> Value.Context.t -> t -> t -> t

    val nconcat: Value.Context.t -> (In_bytes.t * t) list -> t

    val addresses_in_block: memsize:In_bytes.t -> Value.Context.t -> t -> Value.Context.t -> t -> (Value.binary * Value.binary) list
  ...
  module type Array_result = sig
    ...
    (* Converts a cell into an array with a single cell whose
       contents is cell. *)
    val singleton: memsize:In_bytes.t -> Value.Context.t -> cell -> t
    ...
    val subst: memsize:In_bytes.t -> Scalar.Context.t -> Scalar.binary -> In_bytes.t -> t ->
      strong:(memsize:In_bytes.t -> offset:In_bytes.t ->
              min:Operable_Value.Bound.t -> max:Operable_Value.Bound.t ->
              cell -> cell) ->
      weak:(memsize:In_bytes.t -> offset:In_bytes.t ->
            min:Operable_Value.Bound.t -> max:Operable_Value.Bound.t ->
            cell -> cell) ->
      t
    ...
    (* Probably temporary. *)
    val load: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Value.binary

    (* Extract a cell from an array. *)
    val extract: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> cell

    val serialize:widens:bool -> memsize:In_bytes.t ->
  ...
  = struct
    ...
    type cell = Cell.t
      ...
      let addresses_in_block = Cell.addresses_in_block
      ...
      let subst ~(memsize:In_bytes.t) ctx e (k:In_bytes.t) (* ~min ~max *) cell ~strong ~weak =
        ...
        match Scalar.Query.Binary_Lattice.is_singleton ~size:ptr_size bin with
        | Some i -> begin (* Strong update. *)
          let i = Z.to_int i in
          let ki = In_bytes.times i k in
          (* assert(0 <= i);
           * assert(ki + size <= memsize); *)
          let open Bound in
          let min = (* match min with UBound -> UBound | CBound m -> *) CBound (0 - (ki:>int)) in
          let max = (* match max with UBound -> UBound | CBound m -> *) CBound ((memsize:>int) - (ki:>int)) in
        ...
        | None when Scalar.Query.Binary_Lattice.is_empty ~size:ptr_size bin -> raise Sig.Memory_Empty
        | None -> begin
          let inf = Z.of_int 0 and sup = (Z.of_int @@ ((memsize:>int) - 1)/(k:>int)) in
          let res = Scalar.Query.Binary_Lattice.fold_crop_unsigned ~size:ptr_size bin ~inf ~sup cell (fun i acc ->
  ...
  module Make_Array2(Value:VALUE with module Scalar = Scalar and module Context=Scalar.Context)(Cell:Array_Arg with module Value = Value):Array_result
    with type cell = Cell.t
  ...
  = struct
    ...
    let grow ctx array n =
      ...
      in { cell_size=In_bytes.times n array.cell_size;
    ...
    let shrink ctx array n =
      ...
      assert ((oldsize:>int) mod n == 0);
      let size = (oldsize:>int) / n |> In_bytes.of_int in
      let rec loop i acc =
        if i == oldsize then acc
        else
          let acc = Cell.union ~memsize:size ctx acc @@
            Cell.extract ~memsize:oldsize ~offset:i ~size ctx mem in
          loop In_bytes.(i+size) acc
      in
      let new_content = loop size (Cell.extract ~memsize:oldsize ~offset:In_bytes.zero ~size ctx mem) in
    ...
    let resize ctx array (k:In_bytes.t) =
      let oldsize = In_bytes.to_int array.cell_size in
      let k = In_bytes.to_int k in
    ...
    let _resize ctx array k =
      let oldsize = array.cell_size in
      let oldsize_int = In_bytes.to_int array.cell_size in
      let k_int = In_bytes.to_int k in
    ...
    let subst ~memsize ctx e (k:In_bytes.t) array ~strong ~weak =
      ...
      let mem = weak ~memsize:k ~offset:In_bytes.zero ~min:(Operable_Value.Bound.UBound) ~max:(Operable_Value.Bound.UBound) mem in
    ...
    let load ~memsize ~(offset:In_bytes.t) ~(size:In_bytes.t) ctx array =
      let k = array.cell_size in
      let offset = (offset:>int) mod (k:>int) |> In_bytes.of_int in
      let array =
        if In_bytes.(offset + size <= k) then array
        (* Grow the cell to the smallest multiple needed so that the
           load can succeed. *)
        else grow ctx array (((offset:>int) + (size:>int) + ((k:>int) - 1))/(k:>int))
      in
      let v = Cell.load ~memsize:array.cell_size ~offset ~size ctx array.cell_contents in
      Value.Binary_Forward.bchoose ~size:(In_bytes.in_bits size) (Operator.Choice.fresh()) ctx v
    ;;

    let extract ~memsize ~(offset:In_bytes.t) ~size ctx array =
      (* assert false; *)
      let k = array.cell_size in
      let offset = (offset:>int) mod (k:>int) |> In_bytes.of_int in
      let array =
    ...
    let serialize ~widens ~memsize ctxa a ctxb b acc =
      (* We serialize using the largest length, as shrinking brings imprecisions. *)
      let newsize = lcm (a.cell_size:>int) (b.cell_size:>int) |> In_bytes.of_int in
      let a = grow ctxa a ((newsize:>int) / (a.cell_size:>int)) in
      let b = grow ctxb b ((newsize:>int) / (b.cell_size:>int)) in
      ...
      let Context.Result(inc,acc,d) = Cell.serialize ~widens ~memsize:newsize ctxa a.cell_contents ctxb b.cell_contents acc
      in Context.Result(inc, acc, (fun ctx tup -> let x,tup = d ctx tup in {cell_size=newsize;cell_contents=x},tup))
    ;;

    let addresses_in_block ~memsize ctxa a ctxb b =
      let newsize = lcm (a.cell_size:>int) (b.cell_size:>int) |> In_bytes.of_int in
      let a = grow ctxa a ((newsize:>int) / (a.cell_size:>int)) in
      let b = grow ctxb b ((newsize:>int) / (b.cell_size:>int)) in
      assert(a.cell_size == newsize);
      assert(b.cell_size == newsize);
      Cell.addresses_in_block ~memsize:newsize ctxa a.cell_contents ctxb b.cell_contents
    ;;
  ...
  = struct
    ...
    let join_values_nondet ~size ctx v1 v2 =
      let Context.Result(_,tup,deserialize) =
        Value.serialize ~widens:false ~size ctx v1 ctx v2 (true, Context.empty_tuple ())
      in
      let res_tup = Scalar.nondet_same_context ctx tup in
      let res,_ = deserialize ctx res_tup in
      res
    ...
    let join_values_union ~size ctx v1 v2 =
      let Context.Result(_,tup,deserialize) =
        Value.serialize ~widens:false ~size ctx v1 ctx v2 (true, Context.empty_tuple ())
      in
      let condition = Operator.Condition.fresh() in
    ...
    module rec Cell:sig
      ...
      val initial: memsize:In_bytes.t -> Value.Context.t -> t

      (* Reading and writing values out of a cell. *)
      val load: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Value.binary
      (* val store: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Value.binary -> t *)
      val store_cell: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Cell.t -> t
      ...
      val serialize:widens:bool -> memsize:In_bytes.t -> Value.Context.t -> t -> Value.Context.t -> t ->
       ...
       *   memsize_low:In_bytes.t -> Cell.t ->
       *   memsize_high:In_bytes.t -> Cell.t ->
       *   t *)
      ...
      let initial ~memsize ctx =
        let zero = Value.binary_empty ~size:(In_bytes.in_bits memsize) ctx in
        Value zero
      ;;

      let pretty ~memsize ctx fmt = function
        | Value v -> Format.fprintf fmt "Value(%a)" (Value.binary_pretty ~size:(In_bytes.in_bits memsize) ctx) v
      ...
      let serialize ~widens ~memsize ctxa a ctxb b acc =
        ...
        match a,b with
        | Value a, Value b ->
          let Context.Result(inc,acc,d) = Value.serialize ~widens ~size:(In_bytes.in_bits memsize) ctxa a ctxb b acc in
          Context.Result(inc, acc, (fun ctx tup -> let x,tup = d ctx tup in Value x,tup))
        | Struct a, Struct b ->
          let Context.Result(inc,acc, d) = Struct.serialize ~widens ~memsize ctxa a ctxb b acc in
          Context.Result(inc, acc, (fun ctx tup -> let x,tup = d ctx tup in Struct x,tup))
        | Array a, Array b ->
          let Context.Result(inc,acc, d) = Array.serialize ~widens ~memsize ctxa a ctxb b acc in
          Context.Result(inc, acc, (fun ctx tup -> let x,tup = d ctx tup in Array(x),tup))
        (* Decay into an array. *)
        | Value a, Array b ->
          let Context.Result(inc, acc, d) =
            Array.serialize ~widens ~memsize ctxa (Array.singleton ~memsize ctxa @@ Value a) ctxb b acc in
          Context.Result(inc, acc, (fun ctx tup -> let res,tup = d ctx tup in Array res,tup))
        | Array a, Value b ->
          let Context.Result(inc, acc, d) =
            Array.serialize ~widens ~memsize ctxa a ctxb (Array.singleton ~memsize ctxb @@ Value b) acc in
          Context.Result(inc, acc, (fun ctx tup -> let res,tup = d ctx tup in Array res,tup))
        (* Decay into a struct. *)
        | (Value _ | Array _) as a, Struct b ->
          let Context.Result(inc, acc, d) =
            Struct.serialize ~widens ~memsize ctxa (Struct.singleton ~memsize a) ctxb b acc in
          Context.Result(inc, acc, (fun ctx tup -> let res,tup = d ctx tup in Struct res,tup))
        | Struct a, ((Value _ | Array _) as b) ->
          let Context.Result(inc, acc, d) =
            Struct.serialize ~widens ~memsize ctxa a ctxb (Struct.singleton ~memsize b) acc in
          Context.Result(inc, acc, (fun ctx tup -> let res,tup = d ctx tup in Struct res,tup))
        ;;
      ...
      let store_cell  ~memsize ~offset ~size ctx memory cell =
        ...
        match memory with
        | _ when memsize == size -> assert(offset == In_bytes.zero); cell
      ...
      let load ~memsize ~offset ~size ctx memory =
        match memory with
        | Value v when memsize == size -> assert(offset == In_bytes.zero); v
        | Value v ->
          assert In_bytes.(zero <= offset);
          Value.Binary_Forward.bextract ctx v
            ~index:(In_bytes.in_bits offset) ~size:(In_bytes.in_bits size) ~oldsize:(In_bytes.in_bits memsize)
      ...
      let extract ~memsize ~offset ~size ctx cell =
        if offset == In_bytes.zero && size == memsize
        then cell
        else match cell with
          | Value v ->     (* Note: depends on endianness. *)
            assert In_bytes.(zero <= offset);
            let v = Value.Binary_Forward.bextract ctx v
                ~index:(In_bytes.in_bits offset) ~size:(In_bytes.in_bits size) ~oldsize:(In_bytes.in_bits memsize) in
            Value v
      ...
      let union ~memsize ctx m1 m2 =
        let Context.Result(_,tup,deserialize2) = serialize ~widens:false ~memsize ctx m1 ctx m2 (true, Context.empty_tuple ()) in
      ...
      let concat ctx ~memsize_low mem_low ~memsize_high mem_high =
        let memsize = In_bytes.(memsize_low + memsize_high) in
        let mem = Struct.singleton ~memsize (initial ~memsize ctx) in
        let mem = Struct.store ~memsize ~offset:In_bytes.zero ~size:memsize_low ctx mem mem_low in
        let mem = Struct.store ~memsize ~offset:memsize_low ~size:memsize_high ctx mem mem_high in
        Struct mem
      ;;

      let nconcat ctx l =
        let memsize = List.fold_left (fun acc (size,_) -> In_bytes.(acc+size)) In_bytes.zero l in
        let mem = Struct.singleton ~memsize (initial ~memsize ctx) in
        let (_,res) = List.fold_left (fun (offset,acc) (size,cell) ->
            (In_bytes.(offset + size), Struct.store ~memsize ~offset ~size ctx acc cell)
          ) (In_bytes.zero,mem) l in
        Struct res
      ;;

      let addresses_in_block ~(memsize:In_bytes.t) ctxa a ctxb b =
        Log.debug (fun p -> p "Region_suffix_tree.Cell.addresses_in_block ~memsize:%d" (memsize:>int));
        match a,b with
        | Value a, Value b ->
          Value.addresses_in_binary ~size:(In_bytes.in_bits memsize) ctxa a ctxb b
        | Struct a, Struct b ->
          Struct.addresses_in_block ~memsize ctxa a ctxb b
        | Array a, Array b ->
          Array.addresses_in_block ~memsize ctxa a ctxb b
        (* Decay into an array. *)
        | Value a, Array b ->
          Array.addresses_in_block ~memsize ctxa (Array.singleton ~memsize ctxa @@ Value a) ctxb b
        | Array a, Value b ->
          Array.addresses_in_block ~memsize ctxa a ctxb (Array.singleton ~memsize ctxb @@ Value b)
        (* Decay into addresses_in_block struct. *)
        | (Value _ | Array _) as a, Struct b ->
          Struct.addresses_in_block ~memsize ctxa (Struct.singleton ~memsize a) ctxb b
        | Struct a, ((Value _ | Array _) as b) ->
          Struct.addresses_in_block ~memsize ctxa a ctxb (Struct.singleton ~memsize b)
        ;;
    ...
    and Struct:sig
      ...
      val singleton: memsize:In_bytes.t -> Cell.t -> t

      val store: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Cell.t -> t
      val load: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Value.binary

      val pretty:memsize:In_bytes.t -> Value.Context.t -> Format.formatter -> t -> unit

      (* Copying part of a struct. *)
      val extract: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> t -> Cell.t
      ...
      let singleton ~(memsize:In_bytes.t) c = IntervalMap.create ~size:(memsize:>int) c
      ...
      let store ~memsize ~(offset:In_bytes.t) ~(size:In_bytes.t) ctx memory value =
        IntervalMap.store ~size:(size:>int) (offset:>int) memory value


      let pretty ~(memsize:In_bytes.t) ctx fmt map =
        ...
        in
        IntervalMap.iter_between ~size:(memsize:>int) 0 map ~extract:(extract ctx) (fun ~size offset v ->
            Format.fprintf fmt " (%d-%d)" offset (offset+size-1);
            let (v,idx,size,oldsize) = v in
            if size == oldsize && idx == 0
            then Cell.pretty ctx ~memsize:(In_bytes.of_int size) fmt v
            else Format.fprintf fmt "extract(%d,%d,%a)" idx size (Cell.pretty ctx ~memsize:(In_bytes.of_int oldsize)) v
          )
      ;;



      let load ~(memsize:In_bytes.t) ~(offset:In_bytes.t) ~(size:In_bytes.t) (ctx:Value.Context.t) memory =
        let extract ctx v ~idx ~size ~oldsize =
          let idx = In_bytes.of_int idx in
          let size = In_bytes.of_int size in
          let oldsize = In_bytes.of_int oldsize in          
        ...
        | (sizea,a)::rest ->
          ...
          let f (size,acc) (newsize,newv) =
            let newsize = newsize * 8 |> In_bits.of_int in
            let acc = Value.Binary_Forward.bconcat ctx ~size1:size acc ~size2:newsize newv in
            In_bits.(size+newsize,acc)
          in
          let fsize,v = List.fold_left f (In_bytes.(sizea |> of_int |> in_bits),a) rest in
          assert ((fsize:>int) == (size:>int) * 8);
          v
      ;;
      ...
      let extract ~(memsize:In_bytes.t) ~(offset:In_bytes.t) ~(size:In_bytes.t) ctx memory =
        let extract ctx v ~idx ~size ~oldsize =
          let idx = In_bytes.of_int idx in
          let size = In_bytes.of_int size in
          let oldsize = In_bytes.of_int oldsize in                    
        ...
        assert In_bytes.(key >= zero);
        let region_size = IntervalMap.get_size memory |> In_bytes.of_int in
        assert(memsize == region_size);
        (* Codex_log.feedback "loading offset %d size %d region_size %d %a"
           key size region_size (pretty ctx) {map}; *)
        assert In_bytes.(key + size <= region_size);
        ...
        in match l with
        | [] -> assert false    (* The address is correct, we must load something. *)
        | [s,v] -> assert(s == (size:>int)); v
        | _ ->
          (* Note that fold build the list with the right part first. *)
          let extracted_size = size in
          let acc = IntervalMap.create ~size:(extracted_size:>int) (Obj.magic 0) in
          let (offset,acc) = List.fold_left (fun (offset,acc) (size,cell) ->
              let offset = offset - size in
              let acc = store ~memsize:extracted_size ~offset:(In_bytes.of_int offset) ~size:(In_bytes.of_int size) ctx acc cell in
              (offset,acc)
            ) ((extracted_size:>int),acc) l
          in
          assert(offset == 0);
          let acc:t = acc in
      ...
      module ExtractedCell:sig
        ...
        val extract :
          memsize:In_bytes.t ->
          offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t -> Cell.t -> t
      ...
      let serialize ~widens ~memsize ctxa a ctxb b (inc, acc) =
        ...
        IntervalMap.fold_on_diff a b (Context.Result(inc, acc,fun _ctx tup -> a,tup))
          ...
          (fun ~size offset a b (Context.Result(inc,acc,d_map)) ->
             let Context.Result(inc,acc,d_cell) = ExtractedCell.serialize ~widens ~memsize:(In_bytes.of_int size)
      ...
      let addresses_in_block ~memsize ctxa a ctxb b = [] (* TODO : improve this *)
    ...
    let serialize ~widens ctxa a ctxb b acc =
      assert(a.size == b.size);
      let Context.Result(inc,acc,deserialize) =
        Cell.serialize ~widens ~memsize:a.size ctxa a.contents ctxb b.contents acc
    ...
    let addresses_in_block ctxa a ctxb b =
      assert (a.size == b.size);
      Cell.addresses_in_block ~memsize:a.size ctxa a.contents ctxb b.contents
    ...
    module type PtrInteraction = sig
      ...
      val load: min:Bound.t -> max:Bound.t ->
        memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t -> Value.Context.t ->
        Cell.t -> offset -> Value.binary

      val store: memsize:In_bytes.t -> offset:In_bytes.t -> size:In_bytes.t ->
        min:Bound.t -> max:Bound.t ->
        Value.Context.t ->
    ...
    module MakePtrInteraction(Indices:Indices):PtrInteraction = struct
      ...
      let load ~min ~max ~memsize ~offset ~size ctx memory suffix =
        ...
        let rec loop ~min ~max ~memsize ~offset ~(size:In_bytes.t) ctx memory suffix acc =
          ...
          match min,max with
          | Bound.CBound min, Bound.CBound max when min + (size:>int) > max -> acc
          | _ ->
            let open Bound in
            let acc =
              if suffix.leaf &&
                 (match min with UBound -> true | CBound m -> m <= 0) &&
                 (match max with UBound -> true | CBound m -> (size:>int) <= m)
            ...
            let acc = IntMap.fold (fun off suffix acc ->
                ...
                loop ~min ~max ~memsize ~offset:In_bytes.(offset+of_int off) ~size ctx memory suffix acc
              ) suffix.offsets acc
            in
        ...
        match res with
        | [] -> (Value.binary_empty ~size:(In_bytes.in_bits size) ctx)
        | [x] -> x
        | a::b -> List.fold_left (join_values ~size:(In_bytes.in_bits size) ctx) a b
      ;;

      let store ~(memsize:In_bytes.t) ~(offset:In_bytes.t) ~(size:In_bytes.t) ~min ~max ctx memory suffix value =
        let open Operable_Value in
        let rec loop ~min ~max ~memsize ~offset ~(size:In_bytes.t) ctx memory suffix value acc =
          ...
          | _ ->
            ...
            let acc = IntMap.fold (fun off suffix acc ->
                let min = match min with UBound -> UBound | CBound m -> CBound (m - off) in
                let max = match max with UBound -> UBound | CBound m -> CBound (m - off) in
                (loop ~min ~max ~memsize ~offset:In_bytes.(offset+ of_int off) ~size ctx memory suffix value acc)
              ) suffix.offsets acc
            in
            let acc = IntMap.fold (fun k (e,suffix) acc ->
                (* Detect double array indexing; we consider an array of array as a single array
                   (as we can do pointer arithmetics that spans across the whole matrix. *)
                ...
                | false -> begin
                    ...
                    (* Most common case: index is = k * e + ksub * esub + suffixsub, with k a multiple of k/ksub. *)
                    (* We replace it by a single index which is ksub * (esub + e*(k/ksub)) + suffixsub  *)
                    ...
                    assert(k mod ksub == 0); (* One is multiple of the other.  *)
                    (* Cannot do it: suffix.min is relative to the offset. We forget it for now.
                       let (min,max)= Bound.inter (min,max) (suffix.min,suffix.max) in *)
                    let esub =
                      Scalar.Binary_Forward.biadd ~size:ptrsize ~flags:(Operator.Flags.Biadd.pack ~nsw:true ~nuw:false ~nusw:false) ctx esub @@
                      Scalar.Binary_Forward.bimul ~size:ptrsize ~flags:(Operator.Flags.Bimul.pack ~nsw:true ~nuw:false) ctx e @@
                      Scalar.Binary_Forward.biconst ~size:ptrsize (Z.of_int (k/ksub)) ctx
                    in
                    try
    ...
    module MakeIndicesInPlaceEnumeration(PtrInteraction:PtrInteraction):Indices = struct
      ...
      let load ~min ~max ~(memsize:In_bytes.t) ~(offset:In_bytes.t) ~(size:In_bytes.t) (ctx:Value.Context.t) e k suffix mem =
        ...
        if Scalar.Query.Binary_Lattice.is_empty ~size:ptr_size bin
        then Value.binary_empty ~size:(In_bytes.in_bits size) ctx
        else
          (* Note: only works on finite regions. *)
          let inf,sup = match min,max with
            | Bound.CBound min, Bound.CBound max ->
              get_indices_bound ~min ~max ~size:(size:>int) ~k
            | _ -> assert false
          in
          let res = Scalar.Query.Binary_Lattice.fold_crop_unsigned ~size:ptr_size bin
              ...
              let min = match min with UBound -> UBound | CBound m -> CBound (m - ki) in
              let max = match max with UBound -> UBound | CBound m -> CBound (m - ki) in
              let offset = In_bytes.(offset + of_int ki) in
      ...
      let store ~min ~max ~(memsize:In_bytes.t) ~(offset:In_bytes.t) ~(size:In_bytes.t) (ctx:Value.Context.t) e k suffix mem value =
        ...
        | Some i -> begin
          ...
          let ki = k * i in
          assert(0 <= i);
          assert In_bytes.(offset + of_int ki + size <= memsize);
          let open Bound in
          let min = match min with UBound -> UBound | CBound m -> CBound (m - ki) in
          let max = match max with UBound -> UBound | CBound m -> CBound (m - ki) in
          PtrInteraction.store ~min ~max ~memsize ~offset:In_bytes.(offset + of_int ki) ~size ctx mem suffix value
        end
        | None when Scalar.Query.Binary_Lattice.is_empty ~size:ptr_size bin -> raise Sig.Memory_Empty
        | None -> begin
          (* Note: only works on finite regions. *)
          ...
          let res = Scalar.Query.Binary_Lattice.fold_crop_unsigned ~size:ptr_size bin
              ...
              let min = match min with UBound -> UBound | CBound m -> CBound (m - ki) in
              let max = match max with UBound -> UBound | CBound m -> CBound (m - ki) in
              let old = PtrInteraction.load ~min ~max ~memsize ~offset:In_bytes.(offset + of_int ki) ~size ctx acc suffix in
              let value = join_values ~size:(In_bytes.in_bits size) ctx value old in
              try PtrInteraction.store ~min ~max ~memsize ~offset:In_bytes.(offset + of_int ki) ~size ctx acc suffix value
    ...
    module MakeIndicesSeparateEnumeration(PtrInteraction:PtrInteraction):Indices = struct
          ...
          let rec loop ~inf ~sup ~min ~max suffix =
            ...
            | _ -> begin
              ...
              suffix.indices |> IntMap.iter (fun k (e,suffix) ->
                  ...
                  if Scalar.Query.Binary_Lattice.is_bottom ~size:ptr_size bin then ()
                  else
                    let (emin,emax) = Scalar.Query.Binary_Lattice.to_signed_interval ~size:ptr_size bin in
      ...
      let _store ~min ~max ~memsize ~(offset:In_bytes.t) ~(size:In_bytes.t) ctx e k suffix mem value =
        ...
        *)
        let inf,sup = match min,max with
          | Bound.CBound min, Bound.CBound max ->
            In_bytes.(offset + of_int min),In_bytes.(offset + of_int max)
          | _ -> assert false in
        (* Hopefully the array was already extracted, and extract is a
           trivial operation. *)
        let subsize = In_bytes.(sup - inf) in
        ...
        let weak ~memsize ~(offset:In_bytes.t) ~min ~max cell =
          ...
          let join = join_values ~size:(In_bytes.in_bits size) ctx value old in
          PtrInteraction.store ~memsize ~offset ~size ~min ~max ctx cell suffix join
        in
        let arr = Array.subst ~memsize:subsize ctx e k arr ~strong ~weak in
      ...
      ;;
      let store ~min ~max ~memsize ~(offset:In_bytes.t) ~(size:In_bytes.t) (ctx:Value.Context.t) 
          e k suffix mem value =
        (* assert(fits_within ctx suffix 0 k); *)
        if fits_within ctx suffix 0 k
        then
          _store ~min ~max ~memsize ~offset ~size ctx e (k |> In_bytes.of_int) suffix mem value
        else
    ...
    let store ~size ctx {size=memsize;contents} offset value =
      let contents =
        PtrInteraction.store ~memsize ~offset:In_bytes.zero ~size:(size |> In_bits.in_bytes)
          ~min:(Bound.CBound 0)
          ~max:(Bound.CBound (memsize:>int))
    ...
    let load ~size ctx {size=memsize;contents} offset =
      PtrInteraction.load
        ~min:(Bound.CBound 0)
        ~max:(Bound.CBound (memsize:>int))
        ~memsize ~offset:In_bytes.zero ~size:(size |> In_bits.in_bytes) ctx contents offset
...
module MakeFullyExpanded(Scalar: Sig.BASE) = struct
  module Scalar = Scalar
  module M = MakePrev(Scalar)
  module Offset = M.Operable_Value
  module Make_Block(Value:Memory_sig.VALUE with module Scalar = Scalar) =
    Fully_expanded_finite_region.Make_Block(Offset)(Value)
end

module Make(Scalar: Sig.BASE) = struct
  module Scalar = Scalar
  module M = MakePrev(Scalar)
  module Offset = M.Operable_Value
  module Make_Block = M.Make_Block
end
