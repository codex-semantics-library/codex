(**************************************************************************)
(*  This file is part of the Codex semantics library.                     *)
(*                                                                        *)
(*  Copyright (C) 2013-2025                                               *)
(*    CEA (Commissariat à l'énergie atomique et aux énergies              *)
(*         alternatives)                                                  *)
(*                                                                        *)
(*  you can redistribute it and/or modify it under the terms of the GNU   *)
(*  Lesser General Public License as published by the Free Software       *)
(*  Foundation, version 2.1.                                              *)
(*                                                                        *)
(*  It is distributed in the hope that it will be useful,                 *)
(*  but WITHOUT ANY WARRANTY; without even the implied warranty of        *)
(*  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the         *)
(*  GNU Lesser General Public License for more details.                   *)
(*                                                                        *)
(*  See the GNU Lesser General Public License version 2.1                 *)
(*  for more details (enclosed in the file LICENSE).                      *)
(*                                                                        *)
(**************************************************************************)


module Log = Tracelog.Make(struct let category = "Domains.Terms.NonRelational" end);;

(** The heart of the domain is a mapping from term variables to
   their basis. If a variable is absent from the mapping, it means
   that the variable is mapped to the "top" basis. Thus, the empty map
   denotes top, union of domains is intersection of mappings, and
   intersection of domains is union of the mappings. We do not need to
   represent the bottom domain because it is handled by
   term_domain. *)
module type Map_S = sig
  type t
  type key
  type value
  val top: t

  (** We use add when you don't know if the new value is going to be
     smaller than the previous one. This is necessary in forward
     evaluation because the terms can already exist. *)
  val add: key -> value -> t -> t

  (* (** Use replace when you know that the new value is smaller than the
     previous one. Used when propagating assume conditions. *)
  val replace: key -> value -> t -> t *)
  val inter: t -> t -> t
  val join: t -> t -> t

  val includes_or_widen: t -> t -> bool * t

  val pretty: Format.formatter -> t -> unit
  val find: key -> t -> value
end

module type Key = sig
  include PatriciaTree.KEY
  val pretty: Format.formatter -> t -> unit
end

(*TODO: change this name, to avoid confusion with Memory_sig.VALUE*)
module type Value = Lattices.Sig.JOIN_SEMI_LATTICE_WITH_INTER_BOTTOM

module TC = Operator.Function_symbol

module Const_eval = Terms.Const_eval

module OLDC = Terms.Builder

module MakeUF
    (T: Terms.Sig.TERMS)
    (B: Single_value_abstraction.Sig.NUMERIC_ENUM)
    (V: Terms.Relations.GROUP_ACTION
      with type bitvector = B.bitvector
      and type integer = B.integer
      and type boolean = B.boolean
      and type enum = B.enum
      and type ('a, 'b) relation = ('a, 'b) T.Relation.t)
= struct
  module Terms = T

  (** {2 Terms, used as keys of the maps}            *)
  (*********************************************************)

  module BinaryTerm = struct
    type t = TC.binary Terms.t
    let pretty = Terms.pretty
    let get_level = Terms.level
    let to_int = Terms.hash
  end

  module IntegerTerm = struct
    type t = TC.integer Terms.t
    let pretty = Terms.pretty
    let get_level = Terms.level
    let to_int = Terms.hash
  end

  module BooleanTerm = struct
    type t = TC.boolean Terms.t
    let pretty = Terms.pretty
    let get_level = Terms.level
    let to_int = Terms.hash
  end

  module EnumTerm = struct
    type t = TC.enum Terms.t
    let pretty = Terms.pretty
    let get_level = Terms.level
    let to_int = Terms.hash
  end


  (** {2 Implementation of the maps using Patrica Trees} *)
  (*******************************************************)


  module MakeMap(Key:Key)(Value:Value)(*:Map_S with type key = Key.t
                                          and type value = Value.t *) =
  struct
    include PatriciaTree.MakeMap(Key)

    let pretty fmt x =
      Format.fprintf fmt "@[<hov>%a@]"
      (pretty
        ~pp_sep:(fun fmt () -> Format.fprintf fmt ";@ ")
        (fun fmt key value -> Format.fprintf fmt "%a -> %a" Key.pretty key Value.pretty value))
      x

    type nonrec t = Value.t t
    type key = Key.t
    type value = Value.t

    let top = empty
    let inter a b = idempotent_union (fun _ v1 v2 -> Value.inter v1 v2) a b

    (** Since absence of information is top, we use inter here. *)
    let join a b = idempotent_inter (fun _ v1 v2 -> Value.join v1 v2) a b

    let includes_or_widen a b =
      let is_included = ref true in
      let res = idempotent_inter (fun _ v1 v2 ->
          let bool, res = Value.includes_or_widen v1 v2 in
          is_included := !is_included && bool;
          res
        ) a b
      in
      !is_included, res

    (* Sligthtly more optimized versions when we use M3. *)
    let add ~intersect key value map = insert key (function
      | None -> value
      | Some old -> if intersect then Value.inter old value else value) map
  end

  module BooleanMap = MakeMap(BooleanTerm)(B.Boolean_Lattice)
  module IntegerMap = MakeMap(IntegerTerm)(B.Integer_Lattice)

  module EnumMap = MakeMap(EnumTerm)(B.Enum_Lattice)

  (** Binary is special as lattice operations requires to know the size. *)
  module BinaryMap = struct
    module Map = PatriciaTree.MakeMap(BinaryTerm)

    let join a b = Map.idempotent_inter
      (fun (Terms.Binary{size;_}) a b -> B.Bitvector_Lattice.join ~size a b) a b

    let includes_or_widen a b =
      let is_included = ref true in
      let res = Map.idempotent_inter (fun (Terms.Binary{size;_} as cr) v1 v2 ->
          let bool, res = B.Bitvector_Lattice.includes_or_widen ~size ~previous:v1 v2 in
          Log.debug(fun p -> p "includes or widen %a %a %a res (%b,%a)"
                       (BinaryTerm.pretty) cr
                       (B.Bitvector_Lattice.pretty ~size) v1
                       (B.Bitvector_Lattice.pretty ~size) v2
                       bool (B.Bitvector_Lattice.pretty ~size) res);
          is_included := !is_included && bool;
          res
        ) a b  in
      !is_included, res


    let inter a b = Map.idempotent_union
      (fun (Terms.Binary{size;_}) a b -> B.Bitvector_Lattice.inter ~size a b) a b

    let add ~intersect key value map =
      let Terms.Binary{size; _} = key in
      Map.insert key (function
        | None -> value
        | Some old -> if intersect then B.Bitvector_Lattice.inter ~size old value else value) map

    let pretty fmt x =
      Map.iter (fun (Terms.Binary{size;_} as key) value ->
          Format.fprintf fmt "%a -> %a@\n"
            Terms.pretty key
            (B.Bitvector_Lattice.pretty ~size) value) x

    let find = Map.find

    type key = BinaryTerm.t
    type value = B.Bitvector_Lattice.t
    type t = value Map.t
    let top = Map.empty
  end

  module Const_eval = Const_eval.Make(Terms)

  (** {2 Domain implementation}                   *)
  (************************************************)

  let name = "Base_Domain(" ^ B.name ^ ")"

  module B = B

  type binary = TC.binary Terms.t
  type integer = TC.integer Terms.t
  type boolean = TC.boolean Terms.t
  type enum = TC.enum Terms.t

  type t = {
    boolean_map: BooleanMap.t;
    integer_map: IntegerMap.t;
    binary_map: BinaryMap.t;
    enum_map: EnumMap.t;
  }

  let pp_space_iftrue fmt bool = if bool then Format.fprintf fmt ";@ "

  (* this is a bit tedious, because I only want to print a cup between maps
     if they are not empty*)
  let pretty fmt t =
    Format.fprintf fmt "@[<hov>";
    let previous =
      if BooleanMap.is_empty t.boolean_map
      then false
      else (BooleanMap.pretty fmt t.boolean_map; true) in
    let previous =
      if IntegerMap.is_empty t.integer_map
      then previous
      else (Format.fprintf fmt "%a%a" pp_space_iftrue previous IntegerMap.pretty t.integer_map; true) in
    let previous =
      if BinaryMap.Map.is_empty t.binary_map
      then previous
      else (Format.fprintf fmt "%a%a" pp_space_iftrue previous BinaryMap.pretty t.binary_map; true) in
    if EnumMap.is_empty t.enum_map
    then (if not previous then Format.pp_print_string fmt "<empty>")
    else Format.fprintf fmt "%a%a" pp_space_iftrue previous EnumMap.pretty t.enum_map;
    Format.fprintf fmt "@]"

  let equal = (==)

  let top = {
    boolean_map = BooleanMap.top;
    integer_map = IntegerMap.top;
    binary_map = BinaryMap.top;
    enum_map = EnumMap.top
  }

  let inter a b = {
    boolean_map = BooleanMap.inter a.boolean_map b.boolean_map;
    integer_map = IntegerMap.inter a.integer_map b.integer_map;
    binary_map = BinaryMap.inter a.binary_map b.binary_map;
    enum_map = EnumMap.inter a.enum_map b.enum_map;
  }

  let join a b = {
    boolean_map = BooleanMap.join a.boolean_map b.boolean_map;
    integer_map = IntegerMap.join a.integer_map b.integer_map;
    binary_map = BinaryMap.join a.binary_map b.binary_map;
    enum_map = EnumMap.join a.enum_map b.enum_map;
  }

  let includes_or_widen previous next =
    let boolean_cond,boolean_map =
      BooleanMap.includes_or_widen previous.boolean_map next.boolean_map
    in
    let integer_cond,integer_map =
      IntegerMap.includes_or_widen previous.integer_map next.integer_map
    in
    let binary_cond,binary_map =
      BinaryMap.includes_or_widen previous.binary_map next.binary_map
    in
    let enum_cond,enum_map =
      EnumMap.includes_or_widen previous.enum_map next.enum_map
    in
    boolean_cond && integer_cond && binary_cond && enum_cond,
    { boolean_map; integer_map; binary_map; enum_map }

  let boolean_get_cst x =
    match Const_eval.boolean x with
    | exception Const_eval.Empty -> B.Boolean_Lattice.bottom ()
    | exception Const_eval.Not_a_constant -> B.Boolean_Lattice.bottom ()
    | x -> B.Boolean_Lattice.singleton x
  let binary_get_cst x =
    let Terms.Binary{size; _} = x in
    match Const_eval.binary x with
    | exception Const_eval.Empty -> B.Bitvector_Lattice.bottom ~size
    | exception Const_eval.Not_a_constant -> assert false
    | k -> B.Bitvector_Lattice.singleton ~size k
  let integer_get_cst x =
    match Const_eval.integer x with
    | exception Const_eval.Empty -> B.Integer_Lattice.bottom ()
    | exception Const_eval.Not_a_constant -> assert false
    | x -> B.Integer_Lattice.singleton x
  let enum_get_cst x =
    match Const_eval.enum x with
    | exception Const_eval.Empty -> B.Enum_Lattice.bottom ()
    | exception Const_eval.Not_a_constant -> B.Enum_Lattice.bottom ()
    | x -> B.Enum_Lattice.singleton @@ Z.to_int x

  let query_parent (type tp tc) (term : tp Terms.t) map (relation : (tc, tp) Terms.Relation.t) =
    match term with
    | Terms.Binary _ ->
        let value = BinaryMap.find term map.binary_map in
        V.apply_relation value relation BitvectorMapping
    | Terms.Integer _ ->
        let value = IntegerMap.find term map.integer_map in
        V.apply_relation value relation IntegerMapping
    | Terms.Bool _ ->
        let value = BooleanMap.find term map.boolean_map in
        V.apply_relation value relation BooleanMapping
    | Terms.Enum _ ->
        let value = EnumMap.find term map.enum_map in
        V.apply_relation value relation EnumMapping

  let refine_parent ~intersect (type tp vp tc vc)
      (term : tp Terms.t) map
      (relation : (tc, tp) Terms.Relation.t)
      (vc : vc) (proofc : (vc, tc) V.mapping) =
    match term with
    | Terms.Binary _ ->
        let vp =
          try BinaryMap.find term map.binary_map
          with Not_found -> B.Bitvector_Lattice.top ~size:(Terms.size_of term) in
        let vp = V.refine_relation vp vc relation BitvectorMapping proofc in
        begin match vp with
        | Some vp -> (* Map.add performs intersections as needed *)
                    { map with binary_map = BinaryMap.add ~intersect term vp map.binary_map }
        | None -> map
        end
    | Terms.Integer _ ->
        let vp =
          try IntegerMap.find term map.integer_map
          with Not_found -> B.Integer_Lattice.top () in
        let vp = V.refine_relation vp vc relation IntegerMapping proofc in
        begin match vp with
        | Some vp -> { map with integer_map = IntegerMap.add ~intersect term vp map.integer_map }
        | None -> map
        end
    | Terms.Bool _ ->
        let vp =
          try BooleanMap.find term map.boolean_map
          with Not_found -> B.Boolean_Lattice.top () in
        let vp = V.refine_relation vp vc relation BooleanMapping proofc in
        begin match vp with
        | Some vp -> { map with boolean_map = BooleanMap.add ~intersect term vp map.boolean_map }
        | None -> map
        end
    | Terms.Enum _ ->
        let vp =
          try EnumMap.find term map.enum_map
          with Not_found ->
            (* Shoud probably not happen as we don't have relations on
               enums. If it happens, the only enums are enum_const and
               enum_unknown ~size, in which case we should have the
               size. *)
            assert false in
        let vp = V.refine_relation vp vc relation EnumMapping proofc in
        begin match vp with
        | Some vp -> { map with enum_map = EnumMap.add ~intersect term vp map.enum_map }
        | None -> map
        end

  let query_binary term map =
    if Terms.level term == -1
    then binary_get_cst term
    else let NodeThoughRelation(repr, rel) = Terms.UnionFind.find_representative term in
    let Wrap(x, BitvectorMapping) = query_parent repr map rel in
    x
  let query_boolean term map =
    if Terms.level term == -1
    then boolean_get_cst term
    else let NodeThoughRelation(repr, rel) = Terms.UnionFind.find_representative term in
    let Wrap(x, BooleanMapping) = query_parent repr map rel in
    x
  let query_integer term map =
    if Terms.level term == -1
    then integer_get_cst term
    else let NodeThoughRelation(repr, rel) = Terms.UnionFind.find_representative term in
    let Wrap(x, IntegerMapping) = query_parent repr map rel in
    x

  let query_enum term map =
    if Terms.level term == -1
    then enum_get_cst term
    else let NodeThoughRelation(repr, rel) = Terms.UnionFind.find_representative term in
    let Wrap(x, EnumMapping) = query_parent repr map rel in
    x

  (** When [intersect] is true, set value to the inter of the old and new value,
      else set it to the new value*)
  let [@inline] replace ~intersect term v dom proof =
    if Terms.level term == -1
    then [], dom
    else let NodeThoughRelation(repr, rel) = Terms.UnionFind.find_representative term in
    [Terms.Any repr], refine_parent ~intersect repr dom rel v proof
  let [@inline] replace_bool ~intersect a v dom = replace ~intersect a v dom BooleanMapping
  let [@inline] replace_integer ~intersect a v dom = replace ~intersect a v dom IntegerMapping
  let [@inline] replace_binary ~intersect a v dom = replace ~intersect a v dom BitvectorMapping
  let [@inline] replace_enum ~intersect a v dom = replace ~intersect a v dom EnumMapping

  let [@inline] add_boolean ~intersect x value map = replace_bool ~intersect x value map |> snd
  let [@inline] add_integer ~intersect x value map = replace_integer ~intersect x value map |> snd
  let [@inline] add_binary ~intersect x value map = replace_binary ~intersect x value map |> snd
  let [@inline] add_enum ~intersect x value map =
    let enum_map = EnumMap.add ~intersect x value map.enum_map in {map with enum_map}

  module Query = struct
    include B
    module Binary_Lattice = B.Bitvector_Lattice
    let boolean t x = query_boolean x t
    let integer t x = query_integer x t
    let binary ~size t x = query_binary x t
    let enum t x = query_enum x t
  end
  module Integer_Query = struct
    include Query
    let query = integer
  end




  (** {2 Transfer functions}              *)
  (****************************************)

  let [@inline always] copy query add ~src ~dst dom =
    let value = query src dom in
    add ~intersect:false dst value dom

  let boolean_pretty dom fmt x =
    let res = query_boolean x dom in
    B.Boolean_Lattice.pretty fmt res

  let integer_pretty dom fmt x =
    let res = query_integer x dom in
    B.Integer_Lattice.pretty fmt res

  let binary_pretty ~size dom fmt x =
    let res = query_binary x dom in
    (* Format.fprintf fmt "%a -> %a" Terms.pretty x *)
    (B.Bitvector_Lattice.pretty ~size) fmt res

  let enum_pretty dom fmt x =
    let res = query_enum x dom in
    B.Enum_Lattice.pretty fmt res

  let nondet ~doma ~tupa ~domb ~tupb ~tupres =
    let copy_constraints tup dom =
      Immutable_array.fold_left2 (fun dom (Terms.Any nondet) (Terms.Any orig) ->
          match nondet,orig with
          | Terms.((Binary _ as nondet)), Terms.((Binary _ as orig)) ->
            copy query_binary add_binary ~src:orig ~dst:nondet dom
          | Terms.((Integer _ as nondet)), Terms.((Integer _ as orig)) ->
            copy query_integer add_integer ~src:orig ~dst:nondet dom
          | Terms.((Bool _ as nondet)), Terms.((Bool _ as orig)) ->
            copy query_boolean add_boolean ~src:orig ~dst:nondet dom
          | Terms.((Enum _ as nondet)), Terms.((Enum _ as orig)) ->
            copy query_enum add_enum ~src:orig ~dst:nondet dom
          | _ -> assert false
        ) dom tupres tup
    in
    let doma = copy_constraints tupa doma in
    let domb = copy_constraints tupb domb in
    let dom = join doma domb in
    dom

  (**************** Widened fixpoint step ****************)

  let widened_fixpoint_step ~previous ~previous_tup ~next ~next_tup includes ~res_tup =
    Log.debug (fun p -> p "widened_fixpoint_step: previous@\n%a next:@\n%a" pretty previous pretty next);
    let copy_constraints tup dom =
      Immutable_array.fold_left2 (fun dom (Terms.Any nondet) (Terms.Any orig) ->
          Log.debug (fun p -> p "Copying %a to %a" Terms.pretty orig Terms.pretty nondet);
          match nondet,orig with
          | Terms.((Binary _ as nondet)), Terms.((Binary _ as orig)) ->
            copy query_binary add_binary ~src:orig ~dst:nondet dom
          | Terms.((Integer _ as nondet)), Terms.((Integer _ as orig)) ->
            copy query_integer add_integer ~src:orig ~dst:nondet dom
          | Terms.((Bool _ as nondet)), Terms.((Bool _ as orig)) ->
            copy query_boolean add_boolean ~src:orig ~dst:nondet dom
          | _ -> assert false
        ) dom res_tup tup
    in
    let previous_dom = copy_constraints previous_tup previous in
    let next_dom = copy_constraints next_tup next in
    let bool,dom = includes_or_widen previous_dom next_dom in
    dom,includes && bool

  (**************** Old fixpoint step ****************)

  let fixpoint_open() = ()

  type ('term, 'value) mapping =
    | MappingBinary : (TC.binary, B.bitvector) mapping
    | MappingInteger : (TC.integer, B.integer) mapping
    | MappingBoolean : (TC.boolean, B.boolean) mapping
    | MappingEnum : (TC.enum, B.enum) mapping

  (** We use the property that ['term] is either
      {!TC.boolean}, {!TC.binary}, {!TC.integer} or {!TC.enum} and that
      ['value] is the matching {!B.boolean}, {!B.binary}, {!B.integer} or {!B.enum}

      Full correctness will also rely on the property that objects at the
      same indexes of the arrays will always have the same type, so we still need
      a fallthrough case. *)
  type ('term,'value) widened_value = {
    (* constrain: 'term Terms.t; *)
    widened: 'value;
    previous: 'value;
    has_converged: bool;
    proof: ('term, 'value) mapping;
  }

  (** With proper type function in OCaml, we could get rid of this constructor
      and instead use*)
  type widened_value_sum = AnyVW : (_, _) widened_value -> widened_value_sum [@@unboxed]

  let iteration_threshold = 20

  let fixpoint_step ~lvl ~iteration _actual_dom  ~actuals arg_dom ~args final_dom ~finals =
    let n = Immutable_array.length actuals in
    assert (n == Immutable_array.length args);
    assert (n == Immutable_array.length finals);

    let fixpoint_reached = ref true in
    let widened_values = Immutable_array.init n (fun i ->
        let Terms.Any actual = Immutable_array.get actuals i in
        let Terms.Any arg = Immutable_array.get args i in
        let Terms.Any final = Immutable_array.get finals i in
        match actual,arg,final with
        | (Terms.Binary {size=sizeactual} as actual), (Terms.Binary {size=sizearg} as arg), (Terms.Binary {size=sizefinal} as final) ->
          assert(sizeactual == sizearg && sizearg == sizefinal);
          let size = sizeactual in
          let actualv = query_binary actual arg_dom in
          let argv = query_binary arg arg_dom in
          let finalv = query_binary final final_dom in
          let joined = B.Bitvector_Lattice.join ~size actualv finalv in
          let (has_converged,widened) = B.Bitvector_Lattice.includes_or_widen ~size ~previous:argv joined in
          (* Codex_log.feedback "index %d: @[<v>actual: %a %a@ arg: %a %a@ final: %a %a@ has_converged: %b@ widened: %a@]"
          i
          Terms.pretty actual (B.Bitvector_Lattice.pretty ~size) actualv
          Terms.pretty arg (B.Bitvector_Lattice.pretty ~size) argv
          Terms.pretty final (B.Bitvector_Lattice.pretty ~size) finalv
          has_converged (B.Bitvector_Lattice.pretty ~size) widened; *)
          fixpoint_reached := !fixpoint_reached && has_converged;
          AnyVW{ widened; previous=argv; has_converged; proof=MappingBinary }
        | (Terms.Integer _ as actual), (Terms.Integer _ as arg), (Terms.Integer _ as final) ->
          let actualv = query_integer actual arg_dom in
          let argv = query_integer arg arg_dom in
          let finalv = query_integer final final_dom in
          let joined = B.Integer_Lattice.join actualv finalv in
          let (has_converged,widened) = B.Integer_Lattice.includes_or_widen ~previous:argv joined in
          fixpoint_reached := !fixpoint_reached && has_converged;
          AnyVW{ widened; previous=argv; has_converged; proof=MappingInteger }
        | (Terms.Bool _ as actual), (Terms.Bool _ as arg), (Terms.Bool _ as final) ->
          let actualv = query_boolean actual arg_dom in
          let argv = query_boolean arg arg_dom in
          let finalv = query_boolean final final_dom in
          let joined = B.Boolean_Lattice.join actualv finalv in
          let (has_converged,widened) = B.Boolean_Lattice.includes_or_widen ~previous:argv joined in
          fixpoint_reached := !fixpoint_reached && has_converged;
          AnyVW{ widened; previous=argv; has_converged; proof=MappingBoolean }
        | (Terms.Enum _ as actual), (Terms.Enum _ as arg), (Terms.Enum _ as final) ->
          let actualv = query_enum actual arg_dom in
          let argv = query_enum arg arg_dom in
          let finalv = query_enum final final_dom in
          let joined = B.Enum_Lattice.join actualv finalv in
          let (has_converged,widened) = B.Enum_Lattice.includes_or_widen ~previous:argv joined in
          fixpoint_reached := !fixpoint_reached && has_converged;
          AnyVW{ widened; previous=argv; has_converged; proof=MappingEnum }
        | _ -> failwith  "Typing issue"
      ) in
    !fixpoint_reached, (fun ~close res ->
        let res = Immutable_array.fold_left2 (fun dom (Terms.Any constrain) (AnyVW widened_value) ->
            match constrain,widened_value with
            | Terms.Binary _, { widened; previous; has_converged; proof=MappingBinary } ->
              (* To avoid non convergence (see t143.c):
                 if we haven't reached the global fixed point yet, but this specific
                 variable has, we keep the previous value (no shrinking yet). *)
              let value =
                if iteration<iteration_threshold || !fixpoint_reached || has_converged=false
                then widened else previous in
              add_binary ~intersect:true constrain value dom
            | Terms.Integer _, { widened; previous; has_converged; proof=MappingInteger } ->
              let value =
                if iteration<iteration_threshold || !fixpoint_reached || has_converged=false
                then widened else previous in
              add_integer ~intersect:true constrain value dom
            | Terms.Bool _, { widened; previous; has_converged; proof=MappingBoolean } ->
              let value =
                if iteration<iteration_threshold || !fixpoint_reached || has_converged=false
                then widened else previous in
              add_boolean ~intersect:true constrain value dom
            | Terms.Enum _, { widened; previous; has_converged; proof=MappingEnum } ->
              let value =
                if iteration<iteration_threshold || !fixpoint_reached || has_converged=false
                then widened else previous in
              add_enum ~intersect:true constrain value dom
            | _ -> failwith "Typing issue"
          ) arg_dom res widened_values in
        (* Codex_log.feedback "fpstep bool %b @\nres %a @\narg_dom%a@\nfinal_dom%a" close pretty res pretty arg_dom pretty final_dom; *)
        res
      )

  (** {3 Forward propagation}                                      *)
  (*****************************************************************)

  module Domain_Arity = struct
    type 'r ar0 = t -> 'r -> t
    type ('a,'r) ar1 = t -> 'a -> 'r -> t
    type ('a,'b,'r) ar2 = t -> 'a -> 'b -> 'r -> t
    type ('a,'b,'c,'r) ar3 = t -> 'a -> 'b -> 'c -> 'r -> t
    type ('a,'r) variadic = t -> 'a list -> t
  end

  (** Defines transfer functions of the form dom[res <- f(a,b)]. *)

  let ar0_boolean f dom res = add_boolean ~intersect:true res f dom
  let ar0_integer f dom res = add_integer ~intersect:true res f dom
  let ar0_binary _str f dom res = add_binary ~intersect:true res f dom
  let ar0_enum _str f dom res = add_enum ~intersect:true res f dom

  let [@inline always] ar1 query_arg add_res f dom a res =
    let va = query_arg a dom in
    add_res res (f va) dom

  let ar1_boolean_boolean _str = ar1 query_boolean (add_boolean ~intersect:true)
  let ar1_integer_integer = ar1 query_integer (add_integer ~intersect:true)
  let ar1_binary_binary _str = ar1 query_binary (add_binary ~intersect:true)
  let ar1_boolean_binary = ar1 query_boolean (add_binary ~intersect:true)

  let [@inline always] ar2 query_arg add_res f dom a b res =
    let va = query_arg a dom in
    let vb = query_arg b dom in
    add_res res (f va vb) dom

  let ar2_boolean_boolean_boolean _str = ar2 query_boolean (add_boolean ~intersect:true)
  let ar2_integer_integer_boolean = ar2 query_integer (add_boolean ~intersect:true)
  let ar2_integer_integer_integer = ar2 query_integer (add_integer ~intersect:true)
  let ar2_binary_binary_binary _str = ar2 query_binary (add_binary ~intersect:true)
  let ar2_binary_binary_boolean _str = ar2 query_binary (add_boolean ~intersect:true)

  let ar1_enum_boolean _str = ar1 query_enum (add_boolean ~intersect:true)

  module Boolean_Forward = struct
    let (||) = ar2_boolean_boolean_boolean "||" B.Boolean_Forward.(||)
    let (&&) = ar2_boolean_boolean_boolean "&&" B.Boolean_Forward.(&&)
    let (not) = ar1_boolean_boolean "not" B.Boolean_Forward.not
    let true_ = ar0_boolean B.Boolean_Forward.true_
    let false_  = ar0_boolean B.Boolean_Forward.false_
  end

  module Integer_Forward = struct
    let ile = ar2_integer_integer_boolean B.Integer_Forward.ile
    let ieq = ar2_integer_integer_boolean B.Integer_Forward.ieq
    let iconst k = ar0_integer (B.Integer_Forward.iconst k)
    let one  = iconst Z.one
    let zero = iconst Z.zero

    let ixor = ar2_integer_integer_integer B.Integer_Forward.ixor
    let ior  = ar2_integer_integer_integer B.Integer_Forward.ior
    let iand = ar2_integer_integer_integer B.Integer_Forward.iand
    let ishr = ar2_integer_integer_integer B.Integer_Forward.ishr
    let ishl = ar2_integer_integer_integer B.Integer_Forward.ishl
    let imod = ar2_integer_integer_integer B.Integer_Forward.imod
    let idiv = ar2_integer_integer_integer B.Integer_Forward.idiv
    let imul = ar2_integer_integer_integer B.Integer_Forward.imul
    let iadd = ar2_integer_integer_integer B.Integer_Forward.iadd
    let isub = ar2_integer_integer_integer B.Integer_Forward.isub
    let itimes k = ar1_integer_integer (B.Integer_Forward.itimes k)
  end

  module Binary_Forward = struct
    let biconst ~size k = ar0_binary (Z.to_string k) (B.Bitvector_Forward.biconst ~size k)
    let beq    ~size = ar2_binary_binary_boolean "beq" @@ B.Bitvector_Forward.beq ~size
    let biule  ~size = ar2_binary_binary_boolean "biule" @@ B.Bitvector_Forward.biule ~size
    let bisle  ~size = ar2_binary_binary_boolean "bisle"  @@ B.Bitvector_Forward.bisle ~size
    let biadd  ~size ~flags = ar2_binary_binary_binary "biadd" @@ B.Bitvector_Forward.biadd ~size ~flags
    let bisub  ~size ~flags = ar2_binary_binary_binary "bisub" @@ B.Bitvector_Forward.bisub ~size ~flags
    let bimul  ~size ~flags = ar2_binary_binary_binary "bimul" @@ B.Bitvector_Forward.bimul ~size ~flags
    let bshl   ~size ~flags = ar2_binary_binary_binary "bshl" @@ B.Bitvector_Forward.bshl ~size ~flags
    let bxor   ~size = ar2_binary_binary_binary "bxor" @@ B.Bitvector_Forward.bxor ~size
    let bor    ~size = ar2_binary_binary_binary "bor" @@ B.Bitvector_Forward.bor ~size
    let band   ~size = ar2_binary_binary_binary "band" @@ B.Bitvector_Forward.band ~size
    let bashr  ~size = ar2_binary_binary_binary "bashr" @@ B.Bitvector_Forward.bashr ~size
    let blshr  ~size = ar2_binary_binary_binary "blshr" @@ B.Bitvector_Forward.blshr ~size
    let bisdiv ~size = ar2_binary_binary_binary "bisdiv" @@ B.Bitvector_Forward.bisdiv ~size
    let biudiv ~size = ar2_binary_binary_binary "biudiv" @@ B.Bitvector_Forward.biudiv ~size
    let bismod ~size = ar2_binary_binary_binary "bismod" @@ B.Bitvector_Forward.bismod ~size
    let biumod ~size = ar2_binary_binary_binary "biumod" @@ B.Bitvector_Forward.biumod ~size

    let bsext ~size ~oldsize = ar1_binary_binary "bsext" @@ B.Bitvector_Forward.bsext ~size ~oldsize
    let buext ~size ~oldsize = ar1_binary_binary "buext" @@ B.Bitvector_Forward.buext ~size ~oldsize
    let bchoose ~size cond = ar1_binary_binary "bchoose" @@ (fun x -> x)
    let bofbool ~size = ar1_boolean_binary @@ B.Bitvector_Forward.bofbool ~size
    let bconcat ~size1 ~size2 = ar2_binary_binary_binary "bconcat" @@ B.Bitvector_Forward.bconcat ~size1 ~size2
    let bextract ~size ~index ~oldsize = ar1_binary_binary "bextract" @@ B.Bitvector_Forward.bextract ~size ~index ~oldsize
    let valid ~size _ = assert false
    let valid_ptr_arith ~size _ = assert false
    let baddr ~size _  = assert false
    (* let biconst ~size k = ar0_binary (Z.to_string k) (B.Binary_Forward.biconst ~size k) *)
    let buninit ~size _ = assert false
    let bshift ~size ~offset ~max _ = assert false
    let bindex ~size _ = assert false
  end

  module Enum_Forward = struct
    let caseof ~case = ar1_enum_boolean "caseof" @@ B.Enum_Forward.caseof ~case
    let enum_const ~case = ar0_enum "enum_const" @@ B.Enum_Forward.enum_const ~case
  end

  let binary_empty ~size = ar0_binary "empty" @@ B.Bitvector_Lattice.bottom ~size
  let integer_empty = ar0_integer @@ B.Integer_Lattice.bottom ()
  let boolean_empty = ar0_boolean @@ B.Boolean_Lattice.bottom ()
  let enum_empty = ar0_enum "empty" @@ B.Enum_Lattice.bottom ()

  let binary_unknown ~size = ar0_binary "unknown" (B.Bitvector_Lattice.top ~size)
  let integer_unknown = ar0_integer @@ B.Integer_Lattice.top ()
  let boolean_unknown = ar0_boolean @@ B.Boolean_Lattice.top ()
  let enum_unknown ~enumsize = ar0_enum "unknown" @@ B.Enum_Lattice.top ~size:enumsize



  (** {3 Backward propagation}                                      *)
  (******************************************************************)

  module Backward_Interp = struct
    let [@inline] add_new_value replace a va ((acc, map) as pair) =
      match va with
      | None -> pair
      | Some va ->
          let list, dom = replace a va map in
          list @ acc, dom

    let ar1 query_arg query_res replace dom a res f =
      let v = query_arg a dom in
      let vres = query_res res dom in
      let v = f v vres in
      ([], dom) |> add_new_value replace a v

    let ar1_boolean_boolean = ar1 query_boolean query_boolean (replace_bool ~intersect:true)
    let ar1_integer_integer = ar1 query_integer query_integer (replace_integer ~intersect:true)
    let ar1_binary_binary = ar1 query_binary query_binary (replace_binary ~intersect:true)

    let ar1_enum_boolean = ar1 query_enum query_boolean (replace_enum ~intersect:true)

    let ar2 query_arg query_res replace dom a b res f =
      let va = query_arg a dom in
      let vb = query_arg b dom in
      let vres = query_res res dom in
      let va, vb = f va vb vres in
      ([], dom) |> add_new_value replace a va |> add_new_value replace b vb

    let ar2_boolean_boolean_boolean = ar2 query_boolean query_boolean (replace_bool ~intersect:true)
    let ar2_integer_integer_boolean = ar2 query_integer query_boolean (replace_integer ~intersect:true)
    let ar2_integer_integer_integer = ar2 query_integer query_integer (replace_integer ~intersect:true)
    let ar2_binary_binary_boolean = ar2 query_binary query_boolean (replace_binary ~intersect:true)
    let ar2_binary_binary_binary = ar2 query_binary query_binary (replace_binary ~intersect:true)

    (** Refinement of one term. *)
    let backward_interp dom (Terms.Any x)=
      let open Terms in
      let open TC in
      match x with
      | Bool{term=T2{tag=Or;a;b}} -> ar2_boolean_boolean_boolean dom a b x B.Boolean_Backward.(||)
      | Bool{term=T2{tag=And;a;b}} -> ar2_boolean_boolean_boolean dom a b x B.Boolean_Backward.(&&)
      | Bool{term=T2{tag=Ieq;a;b}} -> ar2_integer_integer_boolean dom a b x B.Integer_Backward.ieq
      | Bool{term=T2{tag=Ile;a;b}} -> ar2_integer_integer_boolean dom a b x B.Integer_Backward.ile
      | Bool{term=T1{tag=Not;a}} -> ar1_boolean_boolean dom a x B.Boolean_Backward.not
      | Bool{term=T0 _} -> [], dom

      | Integer{term=T2{tag=Iadd;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.iadd
      | Integer{term=T2{tag=Isub;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.isub
      | Integer{term=T2{tag=Imul;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.imul
      | Integer{term=T2{tag=Idiv;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.idiv
      | Integer{term=T2{tag=Imod;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.imod
      | Integer{term=T2{tag=Ishl;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.ishl
      | Integer{term=T2{tag=Ishr;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.ishr
      | Integer{term=T2{tag=Iand;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.iand
      | Integer{term=T2{tag=Ior;a;b}}  -> ar2_integer_integer_integer dom a b x B.Integer_Backward.ior
      | Integer{term=T2{tag=Ixor;a;b}} -> ar2_integer_integer_integer dom a b x B.Integer_Backward.ixor
      | Integer{term=T1{tag=Itimes k;a}} -> ar1_integer_integer dom a x (B.Integer_Backward.itimes k)
      | Integer{term=T0 _} -> [], dom


      | Bool{term=T2{tag=Beq size;a;b}} ->   ar2_binary_binary_boolean dom a b x @@ B.Bitvector_Backward.beq   ~size
      | Bool{term=T2{tag=Bisle size;a;b}} -> ar2_binary_binary_boolean dom a b x @@ B.Bitvector_Backward.bisle ~size
      | Bool{term=T2{tag=Biule size;a;b}} -> ar2_binary_binary_boolean dom a b x @@ B.Bitvector_Backward.biule ~size
      | Binary{term=T2{tag=Biadd{size;flags};a;b}} ->  ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.biadd  ~size ~flags
      | Binary{term=T2{tag=Bisub{size;flags};a;b}} ->  ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bisub  ~size ~flags
      | Binary{term=T2{tag=Bimul{size;flags};a;b}} ->  ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bimul  ~size ~flags
      | Binary{term=T2{tag=Bshl{size;flags};a;b}} ->   ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bshl   ~size ~flags
      | Binary{term=T2{tag=Bisdiv(size);a;b}} ->         ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bisdiv ~size
      | Binary{term=T2{tag=Bismod(size);a;b}} ->         ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bismod ~size
      | Binary{term=T2{tag=Biudiv(size);a;b}} ->         ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.biudiv ~size
      | Binary{term=T2{tag=Biumod(size);a;b}} ->         ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.biumod ~size
      | Binary{term=T2{tag=Bashr(size);a;b}} ->          ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bashr  ~size
      | Binary{term=T2{tag=Blshr(size);a;b}} ->          ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.blshr  ~size
      | Binary{term=T2{tag=Band(size);a;b}} ->           ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.band   ~size
      | Binary{term=T2{tag=Bor (size);a;b}} ->           ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bor    ~size
      | Binary{term=T2{tag=Bxor(size);a;b}} ->           ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bxor   ~size
      | Binary{term=T2{tag=Bconcat(size1,size2);a;b}} -> ar2_binary_binary_binary dom a b x @@ B.Bitvector_Backward.bconcat ~size1 ~size2
      | Binary{term=T1{tag=Bsext(size);a=Binary{size=oldsize} as a}} -> ar1_binary_binary dom a x @@ B.Bitvector_Backward.bsext ~size ~oldsize
      | Binary{term=T1{tag=Buext(size);a=Binary{size=oldsize} as a}} -> ar1_binary_binary dom a x @@ B.Bitvector_Backward.buext ~size ~oldsize
      | Binary{term=T1{tag=Bextract{size;index;oldsize};a}} ->          ar1_binary_binary dom a x @@ B.Bitvector_Backward.bextract ~size ~index ~oldsize
      | Binary{term=T1{tag=Bchoose(choice,size);a}} -> [], dom
      | Binary{term=T0 _} -> [], dom

      | Bool{term=T1{tag=CaseOf(case);a}} -> ar1_enum_boolean dom a x @@ B.Enum_Backward.caseof ~case

      | Bool{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> [], dom
      | Integer{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> [], dom
      | Binary{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> [], dom
      | Enum{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> [], dom

      (* We can, but do not, propagate across mu. *)
      | Binary{term=Tuple_get(_,(Mu _ | Inductive_vars _))} -> [], dom
      | Integer{term=Tuple_get(_,(Mu _| Inductive_vars _))} -> [], dom
      | Bool{term=Tuple_get(_,(Mu _| Inductive_vars _))} -> [], dom
      | Enum{term=Tuple_get(_,(Mu _| Inductive_vars _))} -> [], dom

      (* We could also propagate across nondet. *)
      | Bool{term=Tuple_get(_,Nondet _)} -> [], dom
      | Binary{term=Tuple_get(_,Nondet _)} -> [], dom
      | Integer{term=Tuple_get(_,Nondet _)} -> [], dom
      | Enum{term=Tuple_get(_,Nondet _)} -> [], dom

      | Binary{term=T2{tag=Bunion _}} -> [], dom
      | Bool{term=T2{tag=BoolUnion}} -> [], dom

      | t -> Codex_log.fatal "backward_interp on %a" Terms.pretty t

  end

  let backward_interp = Backward_Interp.backward_interp


  (** {3 Forward propagation}                                       *)
  (******************************************************************)

  let forward_interp dom (Terms.Any x)=
    let open Terms in
    let open TC in
    match x with
    | Bool{term=T0 _} -> dom
    | Binary{term=T0 _} -> dom
    | Integer{term=T0 _} -> dom

    | Bool{term=T2{tag=Or;a;b}} ->  Boolean_Forward.(||) dom a b x
    | Bool{term=T2{tag=And;a;b}} -> Boolean_Forward.(&&) dom a b x
    | Bool{term=T1{tag=Not;a}} ->   Boolean_Forward.not dom a x


    | Bool{term=T2{tag=Ieq;a;b}} -> Integer_Forward.ieq dom a b x
    | Bool{term=T2{tag=Ile;a;b}} -> Integer_Forward.ile dom a b x
    | Integer{term=T2{tag=Iadd;a;b}} -> Integer_Forward.iadd dom a b x
    | Integer{term=T2{tag=Isub;a;b}} -> Integer_Forward.isub dom a b x
    | Integer{term=T2{tag=Imul;a;b}} -> Integer_Forward.imul dom a b x
    | Integer{term=T2{tag=Idiv;a;b}} -> Integer_Forward.idiv dom a b x
    | Integer{term=T2{tag=Imod;a;b}} -> Integer_Forward.imod dom a b x
    | Integer{term=T2{tag=Ishl;a;b}} -> Integer_Forward.ishl dom a b x
    | Integer{term=T2{tag=Ishr;a;b}} -> Integer_Forward.ishr dom a b x
    | Integer{term=T2{tag=Iand;a;b}} -> Integer_Forward.iand dom a b x
    | Integer{term=T2{tag=Ior;a;b}}  -> Integer_Forward.ior  dom a b x
    | Integer{term=T2{tag=Ixor;a;b}} -> Integer_Forward.ixor dom a b x
    | Integer{term=T1{tag=Itimes k;a}} -> Integer_Forward.itimes k dom a x

    | Bool{term=T2{tag=Beq size;a;b}} ->   Binary_Forward.beq   ~size dom a b x
    | Bool{term=T2{tag=Bisle size;a;b}} -> Binary_Forward.bisle ~size dom a b x
    | Bool{term=T2{tag=Biule size;a;b}} -> Binary_Forward.biule ~size dom a b x
    | Binary{term=T2{tag=Biadd{size;flags};a;b}} ->  Binary_Forward.biadd  ~size ~flags dom a b x
    | Binary{term=T2{tag=Bisub{size;flags};a;b}} ->  Binary_Forward.bisub  ~size ~flags dom a b x
    | Binary{term=T2{tag=Bimul{size;flags};a;b}} ->    Binary_Forward.bimul  ~size ~flags dom a b x
    | Binary{term=T2{tag=Bshl{size;flags};a;b}} ->     Binary_Forward.bshl   ~size ~flags dom a b x
    | Binary{term=T2{tag=Bisdiv(size);a;b}} ->         Binary_Forward.bisdiv ~size           dom a b x
    | Binary{term=T2{tag=Bismod(size);a;b}} ->         Binary_Forward.bismod ~size           dom a b x
    | Binary{term=T2{tag=Biudiv(size);a;b}} ->         Binary_Forward.biudiv ~size           dom a b x
    | Binary{term=T2{tag=Biumod(size);a;b}} ->         Binary_Forward.biumod ~size           dom a b x
    | Binary{term=T2{tag=Bashr(size);a;b}} ->          Binary_Forward.bashr  ~size           dom a b x
    | Binary{term=T2{tag=Blshr(size);a;b}} ->          Binary_Forward.blshr  ~size           dom a b x
    | Binary{term=T2{tag=Band(size);a;b}} ->           Binary_Forward.band   ~size           dom a b x
    | Binary{term=T2{tag=Bor (size);a;b}} ->           Binary_Forward.bor    ~size           dom a b x
    | Binary{term=T2{tag=Bxor(size);a;b}} ->           Binary_Forward.bxor   ~size           dom a b x
    | Binary{term=T2{tag=Bconcat(size1,size2);a;b}} -> Binary_Forward.bconcat ~size1 ~size2  dom a b x
    | Binary{term=T1{tag=Bsext(size);a=Binary{size=oldsize} as a}} -> Binary_Forward.bsext ~size ~oldsize           dom a x
    | Binary{term=T1{tag=Buext(size);a=Binary{size=oldsize} as a}} -> Binary_Forward.buext ~size ~oldsize           dom a x
    | Binary{term=T1{tag=Bextract{size;index;oldsize};a}} ->          Binary_Forward.bextract ~size ~index ~oldsize dom a x
    | Binary{term=T1{tag=Bchoose(choice,size);a}} ->   Binary_Forward.bchoose ~size choice dom a x

    | Bool{term=T1{tag=CaseOf(case);a}} ->  Enum_Forward.caseof ~case dom a x

    | Bool{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> dom
    | Integer{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> dom
    | Binary{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> dom
    | Enum{term=(Mu_formal _|Inductive_var _ | Unknown _) } -> dom

    (* We can, but do not, propagate across mu. *)
    | Binary{term=Tuple_get(_,(Mu _|Inductive_vars _))} -> dom
    | Integer{term=Tuple_get(_,(Mu _|Inductive_vars _))} -> dom
    | Bool{term=Tuple_get(_,(Mu _|Inductive_vars _))} -> dom
    | Enum{term=Tuple_get(_,(Mu _|Inductive_vars _))} -> dom

    (* We could also propagate across nondet. *)
    (* Maybe the "Context" should have a term, and we should propagate when it becomes bottom.  *)
    | Bool{term=Tuple_get(_,Nondet _)} -> dom
    | Binary{term=Tuple_get(_,Nondet _)} -> dom
    | Integer{term=Tuple_get(_,Nondet _)} -> dom
    | Enum{term=Tuple_get(_,Nondet _)} -> dom


    | Binary{term=T2{tag=Bunion _}} -> dom
    | Bool{term=T2{tag=BoolUnion}} -> dom

    | t -> Codex_log.fatal "forward_interp on %a" Terms.pretty t


  (** {3 Backward propagation}                                      *)
  (******************************************************************)

  module MakeWorklist(Comp:sig
      val compare: Terms.any -> Terms.any -> int
    end) = struct

    module Heap = Binary_heap.Make(struct
        type t = Terms.any
        let compare = Comp.compare
      end)

    let create = Heap.create
    let add = Heap.add
    let pop_minimum = Heap.pop_minimum
  end

  module BackwardWorklist = MakeWorklist(struct
      open Terms
      let compare a b = (Any.get_id_int b) - (Any.get_id_int a)
    end)

  module ForwardWorklist = MakeWorklist(struct
      open Terms
      let compare a b = (Any.get_id_int a) - (Any.get_id_int b)
    end)

  let limit = Codex_config.get_back_propagation_limit ()

  let backward_propagate dom worklist fwd_worklist (* constrain *) =
    let limit_count = ref 0 in
    let exception Stop in
    (* Backward propagate until the worklist is empty. *)
    let rec loop dom =
      try
        let (Terms.Any(max_elt) as max_any) = BackwardWorklist.pop_minimum worklist in
        ForwardWorklist.add fwd_worklist max_any;
        (* Possibly: also add the parents to the backward worklist;
           but make sure that elements are processed only once. *)
        (* Terms.Parents.iter_on_parents max_elt (fun parent ->
           *     ForwardWorklist.add fwd_worklist parent cond
           *   ); *)
        (* TODO: If we find bottom, there is probably no need to
           propagate it further; by forward propagation, we should
           (probably? always?) find that the original condition is
           bottom. The only "legitimate" bottom should be the one
           bound to empty; backward propagation to bottom probably
           indicates an empty state. This may be important as backward
           propagation of bottom may propagate bottom up to the
           beginning of the program. *)
        let changed, newdom = backward_interp dom max_any in
        (* Codex_log.feedback "Backward propagation of %a:@\nold:%a@\nnew:%a" *)
        (*   Terms.pretty max_elt pretty dom pretty newdom; *)
        let dom = newdom in
        changed |> List.iter (fun any ->
            let Terms.Any x = any in
            assert (Terms.Any.get_id_int any < Terms.Any.get_id_int max_any);
            (* Note: because we usse a binary heap, if an element is
               already in the list, we add it twice. We could pair
               with a Hash to avoid that if this was problematic (it
               probably is not). *)
            BackwardWorklist.add worklist any;
            incr limit_count;
            if !limit_count > limit
            then (Codex_log.warning "Propagation: limit found"; raise Stop)
          );
        loop dom
      with Binary_heap.Empty | Stop -> dom
    in loop dom
  ;;

  let forward_propagate dom fwd_worklist =
    let rec loop dom =
      try
        let (Terms.Any(max_elt) as max_any) = ForwardWorklist.pop_minimum fwd_worklist in
        let newdom = forward_interp dom max_any in
        (* Codex_log.feedback "Forward propagation of %a:@\nold:%a@\nnew:%a" *)
        (*   Terms.pretty max_elt pretty dom pretty newdom; *)
        loop newdom
      with Binary_heap.Empty -> dom
    in loop dom
  ;;


  let assume dom cond =
    (* let old_dom = dom in *)
    assert(query_boolean cond dom = Lattices.Quadrivalent.Top);

    let dummy = Terms.Any(Terms.Build.Boolean.true_) in
    let worklist = (BackwardWorklist.create ~dummy 50) in
    let fwd_worklist = (ForwardWorklist.create ~dummy 50) in

    let list, dom = replace_bool ~intersect:true cond Lattices.Quadrivalent.True dom in
    List.iter (fun elt -> BackwardWorklist.add worklist elt) list;
    let dom = backward_propagate dom worklist fwd_worklist in
    let dom = forward_propagate dom fwd_worklist in
    (* Codex_log.feedback "after assume %a:@\nold %a@\nnew %a"
        Terms.pretty cond pretty old_dom pretty dom; *)
    Some dom

    let (let*) = Result.bind

    let hash_get x = match Hashtbl.find OLDC.sizes (Terms.hash x) with
      | n -> n
      | exception Not_found -> 1

    let remove (type a) (elt: a Terms.t) dom = match elt with
      | Terms.Binary _ -> { dom with binary_map=BinaryMap.Map.remove elt dom.binary_map }
      | Terms.Integer _ -> { dom with integer_map=IntegerMap.remove elt dom.integer_map }
      | Terms.Bool _ -> { dom with boolean_map=BooleanMap.remove elt dom.boolean_map }
      | Terms.Enum _ -> { dom with enum_map=EnumMap.remove elt dom.enum_map }

    let union dom child parent relation =
      let NodeThoughRelation(old_parent,rel) = Terms.UnionFind.find_representative child in
      match query_parent old_parent dom rel with
      | Wrap(child_value, mapping) ->
          let r = Option.is_none (Terms.UnionFind.check_related child parent) in
          let* () = Terms.UnionFind.union child parent relation in
          if r then (
            incr OLDC.nb_unions;
            let NodeThoughRelation(np,_) =  Terms.UnionFind.find_representative parent in
            Hashtbl.add OLDC.sizes (Terms.hash np) (hash_get np + hash_get old_parent);
            Hashtbl.remove OLDC.sizes (Terms.hash old_parent);
          );
          let _, dom = replace ~intersect:true child child_value dom mapping in
          let NodeThoughRelation(parents_parent, _)  = Terms.UnionFind.find_representative parent in
          (* maybe backpropate here *)
          let dom = if Terms.equal old_parent parents_parent then dom else remove old_parent dom in
          Ok dom
      | exception Not_found ->
          let r = Option.is_none (Terms.UnionFind.check_related child parent) in
          let* () = Terms.UnionFind.union child parent relation in
          if r then (
            incr OLDC.nb_unions;
            let NodeThoughRelation(np,_) =  Terms.UnionFind.find_representative parent in
            Hashtbl.add OLDC.sizes (Terms.hash np) (hash_get np + hash_get old_parent);
            Hashtbl.remove OLDC.sizes (Terms.hash old_parent);
          );
          Ok dom

    let union dom child parent relation =
      match union dom child parent relation with
      | Error old_relation ->
        Log.fatal (fun p -> p "Union between two related terms %a and %a (new relation: %a, old relation: %a)@."
          Terms.pretty child
          Terms.pretty parent
          Terms.Relation.pretty relation
          Terms.Relation.pretty old_relation)
      | Ok dom ->
        Log.debug (fun p -> p "Union %a --(%a)--> %a"
          Terms.pretty child
          Terms.Relation.pretty relation
          Terms.pretty parent); dom
end

module Make
    (T: Terms.Sig.TERMS with type ('a, 'b) Relation.t = ('a, 'b) Terms.Relations.Equality.t)
    (B: Single_value_abstraction.Sig.NUMERIC_ENUM)
= MakeUF(T)(B)(Terms.Relations.Equality.Action(B))
