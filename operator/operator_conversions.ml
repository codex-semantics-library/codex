...
struct
  type boolean = F.boolean
  type bitvector = F.bitvector
  let biadd ~size ~flags = C.ar2 (F.biadd ~size ~flags)
  let bisub ~size ~flags = C.ar2 (F.bisub ~size ~flags)    
  ...
  let bimul ~size ~flags = C.ar2 (F.bimul ~flags ~size)
  ...
  let bshl ~size ~flags  = C.ar2 (F.bshl ~size ~flags)
...
struct
  include Convert_Bitvector_Forward(C)(struct include F type bitvector = binary end)
...
struct
  type boolean = F.boolean
  type enum = F.enum
  let caseof ~case = C.ar1 (F.caseof ~case)
  let enum_const ~case = C.ar0 (F.enum_const ~case)
end
