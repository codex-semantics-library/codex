 ...
 *   if l < 0
 *   then Codex_log.fatal ~current:true
...
(*    let open Int64 in
  ...
  in
  fun x ->
    let max_64_float = (fun x -> x)(* Extlib.id *) max_64_float in
    if x < min_64_float
